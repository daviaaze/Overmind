  ...
  for (const resourceType in store) {
    if (
      resourceType != RESOURCE_ENERGY &&
			(store[(resourceType as ResourceConstant)] || 0) > 0
    ) {
      return true
    }
  }
...
export function bulleted (
  ...
  const prefix =
		(startWithNewLine ? (aligned ? alignedNewline : '\n') : '') + bullet
  if (aligned) {
...
export function toColumns (
  ...
  const valPadding = _.max(_.map(obj, (str) => str.length))!
 ...
 */
export function mergeSum (objects: Array<Record<string, number | undefined>>): Record<string, number> {
...
export function getPosFromString (
...
export function averageBy<T> (
  ...
  if (objects.length == 0) {
    ...
    return _.sumBy(objects, (obj) => iteratee(obj)) / objects.length
  }
}
    ...
    const heapPercent = Math.round(
      (100 *
				(heapStats.total_heap_size +
					heapStats.externally_allocated_size)) /
				heapStats.heap_size_limit
    )
    const heapSize = Math.round(heapStats.total_heap_size / 1048576)
    const externalHeapSize = Math.round(
      heapStats.externally_allocated_size / 1048576
    )
    const heapLimit = Math.round(heapStats.heap_size_limit / 1048576)
    console.log(
			`Heap usage: ${heapSize} MB + ${externalHeapSize} MB of ${heapLimit} MB (${heapPercent}%).`
    )
  }
}
 ...
 */
export function irregularExponentialMovingAverage (
  current: number,
  avg: number,
  dt: number,
  window: number
 ...
 */
export function rotatedMatrix<T> (
  matrix: T[][],
...
export function getStoreUsage (store: StoreDefinition) {
  const resourceList: Record<string, number> = {}
  _.forEach(RESOURCES_ALL, (resource) => {
    if (store[resource] > 0) {
      resourceList[resource] = store[resource]
    }
  })
