...
import { type Colony, ColonyStage } from "../Colony";
import { log } from "../console/log";
import { bodyCost, type CreepSetup } from "../creepSetups/CreepSetup";
...
export class Hatchery extends HiveCluster {
	...
	energyStructures: Array<StructureSpawn | StructureExtension>; // All spawns and extensions
	...
	private productionQueue: Record<number, SpawnOrder[]>;
	...
	constructor(colony: Colony, headSpawn: StructureSpawn) {
		...
		if (this.colony.layout == "bunker") {
			...
			this.energyStructures = ([] as Array<StructureSpawn | StructureExtension>).concat(this.spawns, this.extensions);
		}
	...
	private computeEnergyStructures(): Array<StructureSpawn | StructureExtension> {
		if (this.colony.layout == "bunker") {
			const positions = _.map(energyStructureOrder, coord => getPosFromBunkerCoord(coord, this.colony));
			let spawnsAndExtensions: Array<StructureSpawn | StructureExtension> = [];
			spawnsAndExtensions = spawnsAndExtensions.concat(this.spawns, this.extensions);
			const energyStructures: Array<StructureSpawn | StructureExtension> = [];
	...
	private registerEnergyRequests(): void {
		...
		_.forEach(this.energyStructures, struct => {
			this.transportRequests.requestInput(struct, Priority.NormalLow);
		});
	...
	private spawnCreep(protoCreep: ProtoCreep, options: SpawnRequestOptions = {}): number {
		...
		if (spawnToUse) {
			// if there is a spawn, create the creep
			if (this.colony.bunker?.coreSpawn && spawnToUse.id == this.colony.bunker.coreSpawn.id && !options.directions) {
	...
	private generateProtoCreep(setup: CreepSetup, overlord: Overlord): ProtoCreep {
		...
		const creepBody = setup.generateBody(this.room.energyCapacityAvailable);
	...
	enqueue(request: SpawnRequest): void {
		...
		if (this.canSpawn(protoCreep.body) && protoCreep.body.length > 0) {
			...
			this.productionQueue[priority].push({
				protoCreep,
				options: request.options
			});
		} else {
			log.debug(
				`${this.room.print}: cannot spawn creep ${protoCreep.name} with body ` + `${JSON.stringify(protoCreep.body)}!`
			);
		}
	}
	...
	visuals(coord: Coord): Coord {
		...
		const spawnProgress: Array<[number, number]> = [];
		...
		// Log energy
		Visualizer.text("Energy", { x: boxX, y, roomName: this.room.name });
		Visualizer.barGraph(
		...
		Visualizer.text("Uptime", { x: boxX, y, roomName: this.room.name });
		Visualizer.barGraph(uptime, { x: boxX + 4, y, roomName: this.room.name }, 5);
		y += 1;
		...
		Visualizer.barGraph(overload, { x: boxX + 4, y, roomName: this.room.name }, 5);
		y += 1;
		...
		for (const i in spawning) {
			...
			Visualizer.barGraph(spawnProgress[i], { x: boxX + 4, y, roomName: this.room.name }, 5);
			y += 1;
		}
		return { x, y: y + 0.25 };
	}
