...
export class Hatchery extends HiveCluster {
  ...
  private registerEnergyRequests (): void {
    ...
    if (this.battery) {
      ...
      if (
        this.battery.energy <
				threshold * this.battery.store.getCapacity()
      ) {
  ...
  get nextAvailability (): number {
    if (!this._nextAvailability) {
      const allQueued = _.flatten(
        _.values(this.productionQueue)
      )
      const queuedSpawnTime =
				_.sumBy(allQueued, (order) => order.protoCreep.body.length) *
				CREEP_SPAWN_TIME
      const activeSpawnTime = _.sumBy(this.spawns, (spawn) =>
        spawn.spawning ? spawn.spawning.remainingTime : 0
      )
      this._nextAvailability =
				(activeSpawnTime + queuedSpawnTime) / this.spawns.length
    }
  ...
  private recordStats () {
    ...
    const spawnUsageThisTick =
			...
			this.spawns.length
    const uptime = exponentialMovingAverage(
      spawnUsageThisTick,
      this.memory.stats.uptime,
      CREEP_LIFE_TIME
    )
    const longUptime = exponentialMovingAverage(
      spawnUsageThisTick,
      this.memory.stats.longUptime,
      5 * CREEP_LIFE_TIME
    )
    const overload = exponentialMovingAverage(
