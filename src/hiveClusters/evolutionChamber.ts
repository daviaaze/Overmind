...
export class EvolutionChamber extends HiveCluster {
  ...
  constructor (colony: Colony, terminal: StructureTerminal) {
    ...
    this.memory = Mem.wrap(
      this.colony.memory,
      'evolutionChamber',
      EvolutionChamberMemoryDefaults
    )
    ...
    const range2Labs = _.filter(this.labs, (lab) =>
      _.every(this.labs, (otherLab) => lab.pos.inRangeTo(otherLab, 2))
    )
    const reagentLabCandidates = _.filter(
      range2Labs,
      (lab) => !_.some(restrictedLabs, (l) => l.id == lab.id)
    )
    if (this.colony.bunker && this.colony.labs.length == 10) {
      this.reagentLabs = _.take(
        _.sortBy(
          ...
          (lab) =>
            -1 * lab.pos.findInRange(this.boostingLabs, 1).length
        ),
        2
      )
    } else {
  ...
  private initLabStatus (): void {
    ...
    switch (this.memory.status) {
      ...
      case LabStatus.AcquiringMinerals: // "We acquire more mineralzzz"
        if (_.every(this.colony.abathur.getMissingBasicMinerals([
          this.memory.activeReaction!
        ]), (amount) => amount == 0)) {
      ...
      case LabStatus.LoadingLabs:
        if (
          _.every(
            this.reagentLabs,
            (lab) =>
            lab.mineralType && lab.store[lab.mineralType] >=
								this.memory.activeReaction!.amount &&
							REAGENTS[
                this.memory.activeReaction!.mineralType
							].includes((lab.mineralType as ResourceConstant))
          )
        ) {
      ...
      case LabStatus.UnloadingLabs:
        if (
          _.every(
            [...this.reagentLabs, ...this.productLabs],
            (lab) => lab.mineralAmount == 0
          )
        ) {
          this.memory.status = LabStatus.Idle
          this.memory.statusTick = Game.time
  ...
  private productLabRequests (labs: StructureLab[]): void {
    if (this.memory.activeReaction) {
      ...
      for (const lab of labs) {
        ...
        lab.mineralType && lab.mineralType != mineralType && lab.store[lab.mineralType] > 0
        const labIsFull = lab.mineralType &&
         lab.store[lab.mineralType] == lab.store.getCapacity(lab.mineralType)
        // Empty out incorrect minerals or if it's time to unload or if lab is full
        if (
          (this.memory.status == LabStatus.UnloadingLabs &&
						lab.mineralType && lab.store[lab.mineralType] > 0) ||
					labHasWrongMineral ||
					labIsFull
        ) {
  ...
  private registerRequests (): void {
    ...
    const boostingProductLabs =
      this.productLabs.filter(
      (lab) => this.labReservations[lab.id]
    )
    const reactionProductLabs =
      this.productLabs.filter(
      (lab) => !this.labReservations[lab.id]
    )
    ...
    boostingRefillLabs.forEach((lab) => { this.transportRequests.requestInput(lab, Priority.High) }
    )
    const reactionRefillLabs = reactionProductLabs.filter(
      (lab) => lab.store.energy < lab.store.getCapacity(RESOURCE_ENERGY)
    )
    reactionRefillLabs.forEach((lab) => { this.transportRequests.requestInput(lab, Priority.NormalLow) }
    )
    const reagentRefillLabs =
      this.reagentLabs.filter(
        (lab) => lab.store.energy < lab.store.getCapacity(RESOURCE_ENERGY)
      )
    reagentRefillLabs.forEach((lab) => { this.transportRequests.requestInput(lab, Priority.NormalLow) }
    )

    // Request resources delivered to / withdrawn from each type of lab
    this.reagentLabRequests(
      this.reagentLabs as [StructureLab, StructureLab]
    )
  ...
  static requiredBoostAmount (
    ...
    const numPartsToBeBoosted = _.filter(
      body,
      (part) => part.type == boostParts[boostType]
    ).length
    return (
      LAB_BOOST_MINERAL *
  ...
  canBoost (
    ...
    const boostAmount = EvolutionChamber.requiredBoostAmount(
      body,
      boostType
    )
    if (this.colony.assets[boostType] >= boostAmount) {
      ...
      return (
        Game.market.credits >
				TraderJoe.settings.market.boostCredits +
  ...
  requestBoost (creep: Zerg, boostType: _ResourceConstantSansEnergy): void {
    ...
    this.neededBoosts[boostType] = Math.min(
  ...
  init (): void {
    ...
    for (const mineralType in this.neededBoosts) {
      ...
      if (boostLab) {
        this.reserveLab(
          (mineralType as _ResourceConstantSansEnergy),
          this.neededBoosts[mineralType],
          boostLab
        )
      }
    }
  ...
  run (): void {
    ...
    for (const resourceType in this.neededBoosts) {
      const needAmount = Math.max(
        this.neededBoosts[resourceType] -
					(this.colony.assets[resourceType] || 0),
        0
      )
      if (needAmount > 0) {
    ...
    const missingBasicMinerals =
			this.colony.abathur.getMissingBasicMinerals(queue)
      ...
      for (const lab of this.productLabs) {
        if (lab.cooldown == 0 && !this.labReservations[lab.id]) {
          ...
          if (result == OK) {
            ...
            log.debug(
							`Couldn't run reaction for lab @ ${lab.pos.print}! Result: ${result}`
            )
          }
        }
      }
  ...
  private stats (): void {
    Stats.log(
			`colonies.${this.colony.name}.evolutionChamber.totalProduction`,
			this.memory.stats.totalProduction
    )
    const labUsage =
			_.sumBy(this.productLabs, (lab) => (lab.cooldown > 0 ? 1 : 0)) /
			this.productLabs.length
    this.memory.stats.avgUsage = exponentialMovingAverage(
      labUsage,
