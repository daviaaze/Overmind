...
import { type TerminalNetwork } from "../logistics/TerminalNetwork";
import { TraderJoe } from "../logistics/TradeNetwork";
import { type TransportRequestGroup } from "../logistics/TransportRequestGroup";
import { Mem } from "../memory/Memory";
import { Pathing } from "../movement/Pathing";
import { Priority } from "../priorities/priorities";
import { profile } from "../profiler/decorator";
import { type Reaction } from "../resources/Abathur";
...
interface EvolutionChamberMemory {
	...
	labMineralTypes: Record<string, _ResourceConstantSansEnergy>;
	stats: {
...
export class EvolutionChamber extends HiveCluster {
	...
	private labReservations: Record<Id<Structure>, { mineralType: string; amount: number }>;

	private neededBoosts: Record<string, number>;
	...
	constructor(colony: Colony, terminal: StructureTerminal) {
		...
		const restrictedLabs = this.colony.bunker
			...
			: _.take(
					_.sortBy(this.labs, lab => Pathing.distance(this.terminal.pos, lab.pos)),
					1
				);
		...
		if (this.colony.bunker && this.colony.labs.length == 10) {
			this.reagentLabs = _.take(
				_.sortBy(reagentLabCandidates, lab => -1 * lab.pos.findInRange(this.boostingLabs, 1).length),
				2
			);
		} else {
			this.reagentLabs = _.take(
				_.sortBy(reagentLabCandidates, lab => -1 * neighboringLabs(lab.pos).length),
				2
			);
		}
		...
		const unrestrictedBoostingLabs = _.sortBy(_.difference(this.productLabs, restrictedLabs), lab =>
	...
	private statusTimeoutCheck(): void {
		...
		if (timeout) {
			log.warning(
				`${this.print}: stuck in state ${this.memory.status} for ${ticksInStatus} ticks, ` +
		...
		switch (this.memory.status) {
			case LabStatus.Idle:
				if (this.memory.activeReaction) {
					...
					log.info(
						`${this.colony.room.print}: starting synthesis of ${ing1} + ${ing2} ${rightArrow} ` +
			...
			case LabStatus.AcquiringMinerals: // "We acquire more mineralzzz"
				if (
					_.every(this.colony.abathur.getMissingBasicMinerals([this.memory.activeReaction!]), amount => amount == 0)
				) {
			...
			case LabStatus.LoadingLabs:
				if (
					_.every(
						...
						lab =>
							lab.mineralType &&
							lab.store[lab.mineralType] >= this.memory.activeReaction!.amount &&
							REAGENTS[this.memory.activeReaction!.mineralType].includes(lab.mineralType as ResourceConstant)
					)
				) {
	...
	private productLabRequests(labs: StructureLab[]): void {
		if (this.memory.activeReaction) {
			const { mineralType } = this.memory.activeReaction;
			for (const lab of labs) {
				const labHasWrongMineral = lab.mineralType && lab.mineralType != mineralType && lab.store[lab.mineralType] > 0;
				const labIsFull = lab.mineralType && lab.store[lab.mineralType] == lab.store.getCapacity(lab.mineralType);
				// Empty out incorrect minerals or if it's time to unload or if lab is full
				if (
					(this.memory.status == LabStatus.UnloadingLabs && lab.mineralType && lab.store[lab.mineralType] > 0) ||
					labHasWrongMineral ||
					labIsFull
				) {
	...
	private boosterLabRequests(labs: StructureLab[]): void {
		for (const lab of labs) {
			...
			if (lab.mineralType != mineralType && lab.mineralAmount > 0) {
				this.transportRequests.requestOutput(lab, Priority.NormalHigh, {
					resourceType: lab.mineralType!
				});
			} else {
				this.transportRequests.requestInput(lab, Priority.NormalHigh, {
					resourceType: mineralType as ResourceConstant,
	...
	private registerRequests(): void {
		...
		const boostingProductLabs = this.productLabs.filter(lab => this.labReservations[lab.id]);
		const reactionProductLabs = this.productLabs.filter(lab => !this.labReservations[lab.id]);

		// Handle energy requests for labs with different priorities
		const boostingRefillLabs = boostingProductLabs.filter(
			lab => lab.store.energy < lab.store.getCapacity(RESOURCE_ENERGY)
		);
		boostingRefillLabs.forEach(lab => {
			this.transportRequests.requestInput(lab, Priority.High);
		});
		const reactionRefillLabs = reactionProductLabs.filter(
			lab => lab.store.energy < lab.store.getCapacity(RESOURCE_ENERGY)
		);
		reactionRefillLabs.forEach(lab => {
			this.transportRequests.requestInput(lab, Priority.NormalLow);
		});
		const reagentRefillLabs = this.reagentLabs.filter(lab => lab.store.energy < lab.store.getCapacity(RESOURCE_ENERGY));
		reagentRefillLabs.forEach(lab => {
			this.transportRequests.requestInput(lab, Priority.NormalLow);
		});
	...
	canBoost(body: BodyPartDefinition[], boostType: _ResourceConstantSansEnergy): boolean {
		...
		if (this.colony.assets[boostType] >= boostAmount) {
			...
			return (
				Game.market.credits >
				TraderJoe.settings.market.boostCredits + boostAmount * Overmind.tradeNetwork.priceOf(boostType)
			);
		}
	}
	...
	init(): void {
		...
		for (const mineralType in this.neededBoosts) {
			...
			for (const id in this.labReservations) {
				...
				if (
					this.labReservations[id as Id<Structure>] &&
					this.labReservations[id as Id<Structure>].mineralType == mineralType
				) {
					boostLab = deref(id as Id<Structure>) as StructureLab;
				}
			}
			if (!boostLab) {
			...
			if (boostLab) {
				this.reserveLab(mineralType as _ResourceConstantSansEnergy, this.neededBoosts[mineralType], boostLab);
			}
		}
	...
	run(): void {
		...
		for (const resourceType in this.neededBoosts) {
			...
			if (needAmount > 0) {
				this.terminalNetwork.requestResource(this.terminal, resourceType as ResourceConstant, needAmount, true, 0);
			}
		}
	...
	private drawLabReport(coord: Coord): Coord {
		...
		if (this.memory.status == LabStatus.Synthesizing && activeReaction) {
			const amountDone = _.sum(
				_.map(this.productLabs, lab => (lab.mineralType == activeReaction.mineralType ? lab.mineralAmount : 0))
			);
			...
			Visualizer.barGraph([amountDone, activeReaction.amount], { x: boxX + 4, y, roomName: this.room.name }, 5);
			y += 1;
		} else {
	...
	private stats(): void {
		...
		const labUsage = _.sumBy(this.productLabs, lab => (lab.cooldown > 0 ? 1 : 0)) / this.productLabs.length;
