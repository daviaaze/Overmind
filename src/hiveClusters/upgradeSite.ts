...
export class UpgradeSite extends HiveCluster {
      ...
      if (this.room.storage) {
        ...
        const amountOver = Math.max(
          this.colony.assets.energy -
						UpgradeSite.settings.energyBuffer,
          0
        )
        let upgradePower =
					1 +
					Math.floor(
            amountOver / UpgradeSite.settings.energyPerBodyUnit
					)
  ...
  init (): void {
    ...
    if (
      this.link &&
			this.link.energy < UpgradeSite.settings.linksRequestBelow
    ) {
  ...
  private buildBatteryIfMissing (): void {
    if (!this.battery && !this.findInputConstructionSite()) {
      ...
      if (buildHere) {
        ...
        if (result == OK) {
          log.info(
            `Upgrade site at ${this.pos.print}: building battery at ${buildHere.print}`
          )
        } else {
          log.warning(
