...
export class CommandCenter extends HiveCluster {
	...
	visuals(coord: Coord): Coord {
		...
		if (this.storage) {
			...
			Visualizer.barGraph(this.storage.store.getFreeCapacity(), { x: boxX + 4, y: y, roomName: this.room.name }, 5);
			y += 1;
		}
		if (this.terminal) {
			...
			Visualizer.barGraph(this.terminal.store.getFreeCapacity(), { x: boxX + 4, y: y, roomName: this.room.name }, 5);
			y += 1;
		}
		return { x: x, y: y + 0.25 };
	}
