...
export class CommandCenter extends HiveCluster {
	...
	private registerRequests(): void {
		...
		if (
			this.powerSpawn &&
			this.powerSpawn.energy < this.powerSpawn.energyCapacity
		) {
			this.transportRequests.requestInput(
				this.powerSpawn,
				Priority.NormalLow,
		...
		if (this.nuker) {
			...
			if (
				...
				(this.colony.assets[RESOURCE_GHODIUM] || 0) >=
					2 * Abathur.settings.maxBatchSize
			) {
				this.transportRequests.requestInput(this.nuker, Priority.Low, {
    ...
    const { x } = coord;
		...
		if (this.storage) {
			...
			Visualizer.barGraph(
				this.storage.store.getFreeCapacity(),
		...
		if (this.terminal) {
			...
			Visualizer.barGraph(
				this.terminal.store.getFreeCapacity(),
				{ x: boxX + 4, y: y, roomName: this.room.name },
				5,
			);
			y += 1;
