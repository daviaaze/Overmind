...
export class SporeCrawler extends HiveCluster {
	...
	private registerEnergyRequests() {
		...
		for (const tower of this.towers) {
			if (tower.store.energy < SporeCrawler.settings.requestThreshold) {
				const multiplier = tower.store.energy < SporeCrawler.settings.criticalEnergyThreshold ? 2 : 1;
	...
	run() {
		if (this.room.hostiles.length > 0) {
			...
			const myCreepDamage =
				ATTACK_POWER * _.sumBy(myDefenders, creep => CombatIntel.getAttackPotential(creep)) +
				RANGED_ATTACK_POWER * _.sumBy(myRangedDefenders, creep => CombatIntel.getRangedAttackPotential(creep));
			...
			possibleTargets = possibleTargets.filter(hostile => {
				if (CombatIntel.isEdgeDancing(hostile)) {
					...
					if (isKillable) {
						...
						if (this.colony.bunker?.anchor && hostile.pos.getRangeTo(this.colony.bunker.anchor) <= 6 + 2) {
							return true;
						}
					}
				} else {
		...
		if (nearbyNukeRamparts.length > 0 && this.colony.terminal && this.colony.terminalState != TERMINAL_STATE_REBUILD) {
