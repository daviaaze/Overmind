...
export class GameCache implements ICache {
  ...
  private cacheCreepsByColony () {
    this.creepsByColony = _.groupBy(
      Game.creeps,
      (creep) => creep.memory[_MEM.COLONY]
  ...
  private cacheOverlords () {
    ...
    const creepNamesByOverlord = _.groupBy(
      _.keys(Game.creeps),
      (name) => Game.creeps[name].memory[_MEM.OVERLORD]
    )
    for (const ref in creepNamesByOverlord) {
      // keys: roleName, value: creepNames[]
      this.overlords[ref] = _.groupBy(
        creepNamesByOverlord[ref],
        (name) => Game.creeps[name].memory.role
      )
    }
  }
