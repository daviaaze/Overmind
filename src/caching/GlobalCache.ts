...
export class $ {
		...
		if (!_cache.structures[cacheKey] || Game.time > _cache.expiration[cacheKey]) {
			...
			if ((_cache.accessed[cacheKey] || 0) < Game.time) {
				_cache.structures[cacheKey] = _.compact(
					_.map(_cache.structures[cacheKey] || [], s => Game.getObjectById(s.id))
				);
			...
			if ((_cache.accessed[cacheKey] || 0) < Game.time) {
				if (_.isArray(_cache.things[cacheKey])) {
					_cache.things[cacheKey] = _.compact(
						_.map(_cache.things[cacheKey] as HasID[], s => Game.getObjectById(s.id))
					) as HasID[];
				} else {
					_cache.things[cacheKey] = Game.getObjectById((_cache.things[cacheKey] as HasID).id) as HasID;
				}
				_cache.accessed[cacheKey] = Game.time;
			}
		}
	...
	static refresh<T extends Record<K, undefined | _HasId | _HasId[]>, K extends string>(thing: T, ...keys: K[]): void {
		for (const key of keys) {
			const validator = thing[key];
			if (validator) {
				if (validator instanceof Array) {
					thing[key] = _.compact(validator.map(s => Game.getObjectById(s.id))) as T[K];
				} else {
					thing[key] = Game.getObjectById(validator.id) as T[K];
				}
			}
		}
	}

	static refreshObject<T extends Record<K, Record<string, undefined | _HasId | _HasId[]>>, K extends string>(
		thing: T,
		...keys: K[]
	): void {
		for (const key of keys) {
			if (thing[key] instanceof Object) {
				for (const prop in thing[key]) {
					const propValue = thing[key][prop];
					if (propValue instanceof Array) {
						thing[key][prop] = _.compact(propValue.map(s => Game.getObjectById(s.id))) as T[K][Extract<
							keyof T[K],
							string
						>];
					} else {
						if (propValue) {
							thing[key][prop] = Game.getObjectById(propValue.id) as T[K][Extract<keyof T[K], string>];
						}
					}
				}
			}
