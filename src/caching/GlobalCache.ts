...
export class $ {
      ...
      if ((_cache.accessed[cacheKey] || 0) < Game.time) {
        if (_.isArray(_cache.things[cacheKey])) {
          _cache.things[cacheKey] = _.compact(
            _.map(_cache.things[cacheKey] as HasID[], (s) =>
              Game.getObjectById(s.id)
            )
          ) as HasID[]
        } else {
          _cache.things[cacheKey] = Game.getObjectById(
            (_cache.things[cacheKey] as HasID).id
          ) as HasID
        }
        _cache.accessed[cacheKey] = Game.time
      }
  ...
  static refreshObject<
		T extends Record<K, Record<string, undefined | _HasId | _HasId[]>>,
		K extends string,
	>(thing: T, ...keys: K[]): void {
    for(const key of keys){
      if (thing[key] instanceof Object) {
        for (const prop in thing[key]) {
          ...
          if (propValue instanceof Array) {
            ...
            ) as T[K][Extract<keyof T[K], string>]
          } else {
            if(propValue){
