...
export class CombatPlanner {
	...
	constructor() {
		this.directives = [];
		this.creeps = [];
		Memory.combatPlanner = defaultCombatPlannerMemory;
	}
	...
	private assembleSquads(): void {
		...
		for (const creep of this.creeps) {
			if (!creep.overlord) {
				...
				const creepDirective = (creep.overlord as CombatOverlord).directive;
	...
	static getSiegeAnalysis(room: Room): SiegeAnalysis {
		...
		const avgBarrierHits = room.barriers.length > 0 ? _.sumBy(room.barriers, b => b.hits) / room.barriers.length : 0;
