...
import { CombatIntel, type CombatPotentials } from '../intel/CombatIntel'
...
export interface CombatPlannerMemory {
  ...
  profiles: Record<string, EnemyProfile>

  defenses: Record<string, object>

  sieges: Record<string, {
    analysis?: SiegeAnalysis
  }>

  skirmishes: Record<string, object>
}
...
export class CombatPlanner {
  ...
  private static computeHitsToSpawn (room: Room): number {
    // TODO
    log.error(`computeHitsToSpawn not implemented! ${room}`)
    return 0
  }
  ...
  private assembleSquads (): void {
    ...
    for (const creep of this.creeps) {
      if (!creep.overlord) {
        ...
        const creepDirective = (creep.overlord as CombatOverlord)
          .directive
        if (
          creepDirective &&
  ...
  registerThreat (directive: DefenseDirective): void {
    ...
    if (this.memory.threats[directive.ref]) {
      ...
      if (threat.potentials) {
        ...
        const decayedAttack = exponentialMovingAverage(
          threat.potentials.attack,
          attack,
          THREAT_DECAY_TIMESCALE
        )
        const decayedRangedAttack = exponentialMovingAverage(
          threat.potentials.rangedAttack,
          rangedAttack,
          THREAT_DECAY_TIMESCALE
        )
        const decayedHeal = exponentialMovingAverage(
  ...
  static getSiegeAnalysis (room: Room): SiegeAnalysis {
    ...
    const avgBarrierHits =
			room.barriers.length > 0
        ? _.sumBy(room.barriers, (b) => b.hits) / room.barriers.length
        : 0
