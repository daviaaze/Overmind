...
export class ExpansionEvaluator {
  static refreshExpansionData (colony: Colony): void {
    ...
    if (
      _.keys(colony.memory.expansionData.possibleExpansions.keys).length ==
				0 ||
    ...
    for (const roomName in colony.memory.expansionData.possibleExpansions) {
      if (
        ...
        if (Memory.rooms[roomName]) {
          const expansionData =
						Memory.rooms[roomName][_RM.EXPANSION_DATA]
          if (expansionData == false) {
            colony.memory.expansionData.possibleExpansions[
              roomName
            ] = false
          } else if (expansionData?.score) {
            colony.memory.expansionData.possibleExpansions[
              roomName
            ] = expansionData.score
          }
  ...
  static computeExpansionData (room: Room, verbose = false): boolean {
    ...
    for (const roomName of possibleOutposts) {
      ...
      outpostSourcePositions[roomName] = _.map(
        roomMemory[_RM.SOURCES],
        (obj) => derefCoords(obj.c, roomName)
      )
    }
    ...
    const outpostScores: Record<string, number> = {}
    ...
    for (const roomName in outpostSourcePositions) {
      ...
      for (const position of sourcePositions) {
        ...
        if (
          ret.incomplete ||
					ret.path.length > Colony.settings.maxSourceDistance
        ) {
    ...
    for (const roomName of roomsByScore) {
      ...
      totalScore += outpostScores[roomName] * factor
    ...
    if (
      !room.memory[_RM.EXPANSION_DATA] ||
			totalScore > (room.memory[_RM.EXPANSION_DATA]).score
    ) {
      room.memory[_RM.EXPANSION_DATA] = {
        score: totalScore,
