...
export class NukePlanner {
	...
	private evaluateRoom(room: Room) {
		...
		const scanStructures = room.structures.filter(s => structureScores[s.structureType]);

		const structureScore = (structure: Structure) =>
			_.sumBy(structure.pos.findInRange(scanStructures, 2), s => structureScores[s.structureType] || 0);

		const bestStructure = maxBy(scanStructures, structure =>
			!structure.pos.lookFor(LOOK_NUKES) ? structureScore(structure) : 0.7 * structureScore(structure)
		);
	...
	private handleNuker(colony: Colony): RoomPosition | undefined {
		...
		const targetRoomName = minBy(_.keys(this.memory.targetRooms), roomName => {
			if (
				Game.map.getRoomLinearDistance(colony.name, roomName) <= NUKE_RANGE &&
