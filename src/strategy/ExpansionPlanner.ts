...
export class ExpansionPlanner implements IExpansionPlanner {
  ...
  private getBestExpansionRoomFor (
    colony: Colony
  ): { roomName: string, score: number } | undefined {
    const allColonyRooms = _.fromPairs(
    ...
    for (const roomName in colony.memory.expansionData.possibleExpansions) {
      let score = colony.memory.expansionData.possibleExpansions[
        roomName
      ] as number | boolean
      ...
      if (score + MAX_SCORE_BONUS > bestScore) {
        ...
        if (
          _.some(range2Rooms, (roomName) => allColonyRooms[roomName])
        ) {
        ...
        if (
          _.some(range3Rooms, (roomName) => allColonyRooms[roomName])
        ) {
        ...
        if (
          _.some(
            adjacentRooms,
            (roomName) => Memory.rooms[roomName][_RM.AVOID]
          )
        ) {
          continue
        }
