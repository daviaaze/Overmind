...
import { type DirectiveExtract } from '../../directives/resource/extract'
...
export class ExtractorOverlord extends Overlord {
  ...
  private populateStructures () {
    if (Game.rooms[this.pos.roomName]) {
      this.extractor = this.pos.lookForStructure(STRUCTURE_EXTRACTOR) as
				| StructureExtractor
				| undefined
    ...
    if (this.container) {
      if (
        this.container.store.getUsedCapacity() >
					0.5 * this.container.store.getCapacity() ||
  ...
  init () {
    const amount =
			this.mineral && this.mineral.mineralAmount > 0
        ? this.mineral.pos.availableNeighbors().length
        : 0
    this.wishlist(
      Math.min(amount, ExtractorOverlord.settings.maxDrones),
      Setups.drones.extractor
    )
  ...
  private handleDrone (drone: Zerg): void {
    ...
    if (drone.room == this.room && !drone.pos.isEdge) {
      if (drone.carry.getUsedCapacity() == 0) {
