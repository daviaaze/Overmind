...
export class ExtractorOverlord extends Overlord {
	...
	private registerOutputRequests(): void {
		if (this.container) {
			if (
				this.container.store.getUsedCapacity() > 0.5 * this.container.store.getCapacity() ||
	...
	private buildOutputIfNeeded(): void {
		...
		if (!this.container) {
			const containerSite = _.first(
				_.filter(this.pos.findInRange(FIND_MY_CONSTRUCTION_SITES, 2), site => site.structureType == STRUCTURE_CONTAINER)
			);
			if (!containerSite) {
