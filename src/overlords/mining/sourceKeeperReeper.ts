...
import { type DirectiveSKOutpost } from "../../directives/colony/outpostSK";
...
import { type OverlordMemory } from "../Overlord";
...
export class SourceReaperOverlord extends CombatOverlord {
	...
	private computeTargetLair() {
		this.targetLair = this.memory.targetLairID ? (deref(this.memory.targetLairID) as StructureKeeperLair) : undefined;
	...
	init() {
		const defenderAmount =
			this.room && (this.room.invaders.length > 0 || RoomIntel.isInvasionLikely(this.room)) ? 1 : 0;
	...
	run() {
		this.autoRun(this.reapers, reaper => {
			this.handleReaper(reaper);
		});
