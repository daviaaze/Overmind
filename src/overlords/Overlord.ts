...
export abstract class Overlord {
	...
	recalculateCreeps(): void {
		...
		this._creeps = _.mapValues(Overmind.cache.overlords[this.ref], creepsOfRole =>
	...
	private synchronizeZerg(role: string, notifyWhenAttacked?: boolean): void {
		...
		const zergNames = _.fromPairs(_.map(this._zerg[role] || [], zerg => [zerg.name, true])) as Record<string, boolean>;
		const creepNames = _.fromPairs(_.map(this._creeps[role] || [], creep => [creep.name, true])) as Record<
			string,
			boolean
		>;
	...
	private synchronizeCombatZerg(role: string, notifyWhenAttacked?: boolean): void {
		...
		const zergNames = _.fromPairs(_.map(this._combatZerg[role] || [], zerg => [zerg.name, true])) as Record<
			string,
			boolean
		>;
		const creepNames = _.fromPairs(_.map(this._creeps[role] || [], creep => [creep.name, true])) as Record<
		...
		for (const creep of this._creeps[role] || []) {
			if (!zergNames[creep.name]) {
				if (Overmind.zerg[creep.name] && (Overmind.zerg[creep.name] as CombatZerg).isCombatZerg) {
	...
	canBoostSetup(setup: CreepSetup): boolean {
		if (this.colony.evolutionChamber && this.boosts[setup.role] && this.boosts[setup.role]!.length > 0) {
			...
			return _.every(this.boosts[setup.role], boost => this.colony.evolutionChamber!.canBoost(body, boost));
		}
		return false;
	}
	...
	shouldBoost(creep: Zerg): boolean {
		...
		if (!evolutionChamber || (creep.ticksToLive && creep.ticksToLive < MIN_LIFETIME_FOR_BOOST * creep.lifetime)) {
			return false;
		}
		...
		if (this.boosts[creep.roleName]) {
			...
			if (boosts.length > 0) {
				return _.every(boosts, boost => evolutionChamber.canBoost(creep.body, boost));
			}
		}
		return false;
	}
