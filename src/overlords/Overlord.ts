...
import { type SpawnRequest, type SpawnRequestOptions } from '../hiveClusters/hatchery'
...
export function hasColony (
  ...
  return (initializer as OverlordInitializer).colony != undefined
}
...
export abstract class Overlord {
  ...
  private _zerg: Record<string, Zerg[]>
  private _combatZerg: Record<string, CombatZerg[]>
  private boosts: Record<string, _ResourceConstantSansEnergy[] | undefined>

  creepUsageReport: Record<string, [number, number] | undefined>
  ...
  constructor (
    ...
    this.boosts = _.mapValues(this._creeps, () => undefined)
  ...
  suspendFor (ticks: number) {
    Overmind.overseer.suspendOverlordFor(this, ticks)
  }

  suspendUntil (untilTick: number) {
    Overmind.overseer.suspendOverlordUntil(this, untilTick)
  }
  ...
  private synchronizeZerg (role: string, notifyWhenAttacked?: boolean): void {
    ...
    const zergNames = _.fromPairs(
      _.map(this._zerg[role] || [], (zerg) => [zerg.name, true])
    ) as Record<string, boolean>
    const creepNames = _.fromPairs(
  ...
  private synchronizeCombatZerg (
    ...
    for (const creep of this._creeps[role] || []) {
      if (!zergNames[creep.name]) {
        if (
          Overmind.zerg[creep.name] &&
					(Overmind.zerg[creep.name] as CombatZerg).isCombatZerg
        ) {
    ...
    for (const creep of creeps) {
      ...
      if (creep.isIdle && creep.canExecute('move')) {
        if (this.colony.hatchery) {
          ...
          if (
            creep.pos.getRangeTo(this.colony.hatchery.pos) <
						hatcheryRestrictedRange
          ) {
            const hatcheryBorder =
							this.colony.hatchery.pos.getPositionsAtRange(
                hatcheryRestrictedRange
							)
            const moveToPos =
							creep.pos.findClosestByRange(hatcheryBorder)
  ...
  canBoostSetup (setup: CreepSetup): boolean {
    if (
      this.colony.evolutionChamber &&
			this.boosts[setup.role] &&
			this.boosts[setup.role]!.length > 0
    ) {
      let energyCapacityAvailable: number
      if (this.spawnGroup) {
        energyCapacityAvailable =
					this.spawnGroup.energyCapacityAvailable
      } else if (this.colony.spawnGroup) {
        energyCapacityAvailable =
					this.colony.spawnGroup.energyCapacityAvailable
      } else if (this.colony.hatchery) {
        energyCapacityAvailable =
					this.colony.hatchery.room.energyCapacityAvailable
      } else {
        return false
      }
      const body = _.map(
        setup.generateBody(energyCapacityAvailable),
        (part) => ({ type: part, hits: 100 })
      )
      if (body.length == 0) return false
      return _.every(this.boosts[setup.role], (boost) =>
        this.colony.evolutionChamber!.canBoost(body, boost)
      )
    }
    return false
  }
  ...
  shouldBoost (creep: Zerg): boolean {
    ...
    if (
      !evolutionChamber ||
			(creep.ticksToLive &&
				creep.ticksToLive < MIN_LIFETIME_FOR_BOOST * creep.lifetime)
    ) {
      return false
    }
    ...
    if (this.boosts[creep.roleName]) {
      const boosts = _.filter(
        this.boosts[creep.roleName],
        (boost) =>
          (creep.boostCounts[boost] || 0) <
					creep.getActiveBodyparts(boostParts[boost])
      )
      if (boosts.length > 0) {
        return _.every(boosts, (boost) =>
          evolutionChamber.canBoost(creep.body, boost)
        )
      }
    }
    return false
...
export function setOverlord (creep: Zerg | Creep, newOverlord: Overlord | null) {
  ...
  log.info(
		`${creep.name} has been reassigned from ${oldOverlord ? oldOverlord.print : 'IDLE'} ` +
