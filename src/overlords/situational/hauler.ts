...
export class HaulingOverlord extends Overlord {
	...
	constructor(
		directive: DirectiveHaul,
		priority = directive.hasDrops ? OverlordPriority.collectionUrgent.haul : OverlordPriority.collection.haul
	) {
	...
	init() {
		...
		const haulingPowerNeeded =
			Math.min(
				...
				this.colony.storage.store.getCapacity() - this.colony.storage.store.getUsedCapacity()
			) * tripDistance;
		...
		if (hauler.carry.getUsedCapacity() === 0) {
			...
			if (hauler.inSameRoomAs(this.directive)) {
				...
				if (this.directive.storeStructure) {
					...
					if (isStoreStructure(this.directive.storeStructure)) {
						store = _.fromPairs(Object.entries(this.directive.storeStructure.store)) as Record<string, number>;
					} else {
						store = {
					...
					for (const resourceType in store) {
						if (store[resourceType] > 0) {
							hauler.task = Tasks.withdraw(this.directive.storeStructure, resourceType as ResourceConstant);
							return;
						}
					}
				}
			...
			if (hauler.inSameRoomAs(this.colony)) {
				...
				for (const resourceType in hauler.carry) {
					if (hauler.carry[resourceType as ResourceConstant] == 0) {
						continue;
					}
					if (resourceType == RESOURCE_ENERGY) {
						...
						if (this.colony.terminal && this.colony.terminal.store.getUsedCapacity() < TERMINAL_CAPACITY) {
							hauler.task = Tasks.transfer(this.colony.terminal, resourceType as ResourceConstant);
							return;
						} else if (this.colony.storage && this.colony.storage.store.getUsedCapacity() < STORAGE_CAPACITY) {
							hauler.task = Tasks.transfer(this.colony.storage, resourceType as ResourceConstant);
							return;
						}
					}
				}
