...
export class OutpostDefenseOverlord extends CombatOverlord {
	...
	init() {
		...
		let healerAmount =
			enemyHealers > 0 || mode == "EARLY" ? this.computeNeededHealerAmount(CombatSetups.healers.default, heal) : 0;
	...
	run() {
		this.autoRun(this.broodlings, broodling => {
			this.handleCombat(broodling);
		});
		this.autoRun(this.hydralisks, mutalisk => {
			this.handleCombat(mutalisk);
		});
