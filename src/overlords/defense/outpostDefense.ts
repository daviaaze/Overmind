...
import { type CreepSetup, patternCost } from '../../creepSetups/CreepSetup'
import { CombatSetups, Roles } from '../../creepSetups/setups'
import { type DirectiveOutpostDefense } from '../../directives/defense/outpostDefense'
...
export class OutpostDefenseOverlord extends CombatOverlord {
  ...
  private handleHealer (healer: CombatZerg): void {
    if (
      ...
      if (
        this.room &&
				_.some(
          [...this.broodlings, ...this.hydralisks],
  ...
  private computeNeededHydraliskAmount (
    setup: CreepSetup,
    enemyRangedPotential: number
  ): number {
    const hydraliskPotential = setup.getBodyPotential(
      RANGED_ATTACK,
      this.colony
    )
  ...
  private computeNeededBroodlingAmount (
    setup: CreepSetup,
    enemyAttackPotential: number
  ): number {
  ...
  init () {
    const maxCost = Math.max(
      patternCost(CombatSetups.hydralisks.default),
      patternCost(CombatSetups.broodlings.default)
    )
    const mode =
			this.colony.room.energyCapacityAvailable >= maxCost
        ? 'NORMAL'
        : 'EARLY'
    ...
    const hydraliskAmount = this.computeNeededHydraliskAmount(
      hydraliskSetup,
      rangedAttack
    )
    this.wishlist(hydraliskAmount, hydraliskSetup, {
      priority: this.priority - 0.2,
      reassignIdle: true
    })

    const broodlingSetup =
			mode == 'NORMAL'
        ? CombatSetups.broodlings.default
        : CombatSetups.broodlings.early
    const broodlingAmount = this.computeNeededBroodlingAmount(
      broodlingSetup,
      attack
    )
    this.wishlist(broodlingAmount, broodlingSetup, {
      priority: this.priority - 0.1,
      reassignIdle: true
    })

    const enemyHealers = _.filter(
      this.room ? this.room.hostiles : [],
      (creep) => CombatIntel.isHealer(creep)
    ).length
    let healerAmount =
			enemyHealers > 0 || mode == 'EARLY'
        ? this.computeNeededHealerAmount(
    ...
    this.wishlist(healerAmount, CombatSetups.healers.default, {
