...
import { type DirectiveInvasionDefense } from "../../directives/defense/invasionDefense";
...
export class RangedDefenseOverlord extends CombatOverlord {
	...
	constructor(directive: DirectiveInvasionDefense, boosted = false, priority = OverlordPriority.defense.rangedDefense) {
		...
		this.hydralisks = this.combatZerg(Roles.ranged, {
			boostWishlist: boosted
				? [boostResources.tough[3], boostResources.ranged_attack[3], boostResources.heal[3], boostResources.move[3]]
				: undefined
		});
	}
	...
	private computeNeededHydraliskAmount(setup: CreepSetup, boostMultiplier: number): number {
		...
		const hydraliskDamage = RANGED_ATTACK_POWER * boostMultiplier * setup.getBodyPotential(RANGED_ATTACK, this.colony);
		...
		const worstDamageMultiplier = _.min(
			_.map(this.room.hostiles, creep => CombatIntel.minimumDamageTakenMultiplier(creep))
		)!;
		return Math.ceil(0.5 + (1.5 * healAmount) / (worstDamageMultiplier * (hydraliskDamage + towerDamage + 1)));
	}
