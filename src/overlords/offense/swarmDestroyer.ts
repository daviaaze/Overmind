...
export class SwarmDestroyerOverlord extends SwarmOverlord {
  ...
  makeSwarms (): void {
    ...
    for (const ref in meleeZergBySwarm) {
      if (ref != undefined) {
        if (DEBUG) {
          log.debug(
						`Making swarm for ${_.map(meleeZergBySwarm[ref], (z) => z.name)}`
          )
        }
  ...
  private handleSwarm (
    ...
    const attack = _.sumBy(nearbyHostiles, (creep) =>
      CombatIntel.getAttackDamage(creep)
    )
    const rangedAttack = _.sumBy(nearbyHostiles, (creep) =>
  ...
  init () {
    ...
    const zerglingPriority =
        ...
        ? this.priority - 0.1
        : this.priority + 0.1
    const zerglingSetup = this.canBoostSetup(
