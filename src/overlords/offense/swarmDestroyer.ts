...
import { type DirectiveSwarmDestroy } from "../../directives/offense/swarmDestroy";
...
export class SwarmDestroyerOverlord extends SwarmOverlord {
		...
		for (let i = 0; i < _.keys(this.swarms).length + 1; i++) {
			this.assemblyPoints.push(
				$.pos(
					this,
					`assemble_${i}`,
					() => this.intel.findSwarmAssemblyPointInColony({ width: 2, height: 2 }, i + 1),
					200
				)!
			);
		}
	...
	private handleSwarm(swarm: Swarm, index: number, waypoint = this.directive.pos) {
		...
		const attack = _.sumBy(nearbyHostiles, creep => CombatIntel.getAttackDamage(creep));
		const rangedAttack = _.sumBy(nearbyHostiles, creep => CombatIntel.getRangedAttackDamage(creep));
		...
		const canPopShield =
			...
			_.min(swarm.creeps.map(creep => 100 * creep.getActiveBodyparts(TOUGH)))!;
	...
	init() {
		...
		const zerglingSetup = this.canBoostSetup(CombatSetups.zerglings.boosted_T3)
			? CombatSetups.zerglings.boosted_T3
		...
		//     ? this.priority - 0.1
		//     : this.priority + 0.1
		// const hydraliskSetup = this.canBoostSetup(
		//   CombatSetups.hydralisks.siege_T3
		// )
		//   ? CombatSetups.healers.boosted_T3
		//   : CombatSetups.healers.default
