...
import { type DirectivePairDestroy } from "../../directives/offense/pairDestroy";
...
export class PairDestroyerOverlord extends Overlord {
	...
	private handleSquad(attacker: CombatZerg): void {
		...
		// Case 2: you have an active healer
			...
			if (
				attacker.needsToRecover(PairDestroyerOverlord.settings.retreatHitsPercent) ||
	...
	init() {
		...
		const attackerSetup = this.canBoostSetup(CombatSetups.zerglings.boosted_T3)
