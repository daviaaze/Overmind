...
import { type DirectiveControllerAttack } from "../../directives/offense/controllerAttack";
...
export class ControllerAttackerOverlord extends Overlord {
	...
	assignments: Record<string, RoomPosition>;
	readyTick: number;
	...
	private getPositionAssignments(): Record<string, RoomPosition> {
		const assignments: Record<string, RoomPosition> = {};
	...
	run() {
		...
		if (this.room?.controller && !this.room.controller.upgradeBlocked) {
			if (
				_.every(this.controllerAttackers, creep => creep.pos.isEqualTo(this.assignments[creep.name])) ||
				_.some(
					this.controllerAttackers,
					creep => creep.pos.isNearTo(this.room!.controller!) && (creep.ticksToLive || 10) <= 2
				)
			) {
				this.launchAttack();
			}
