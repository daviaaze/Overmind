...
export class CommandCenterOverlord extends Overlord {
	...
	init() {
		...
		if (this.colony.bunker?.coreSpawn && this.colony.level == 8 && !this.colony.roomPlanner.memory.relocating) {
				...
				// Otherwise withdraw as much as you can hold
					...
					if (
						...
						(request.resourceType != RESOURCE_ENERGY ||
							...
							this.commandCenter.terminal.store.energy > Energetics.settings.terminal.energy.equilibrium)
					) {
						withdrawFrom = this.commandCenter.terminal;
					}
					manager.task.fork(
						Tasks.withdraw(withdrawFrom, request.resourceType, withdrawAmount, { nextPos: request.target.pos })
					);
				}
			}
			return true;
	...
	private withdrawActions(manager: Zerg): boolean {
		if (manager.carry.getUsedCapacity() < manager.carry.getCapacity()) {
			...
			if (request) {
				const amount = Math.min(request.amount, manager.carry.getFreeCapacity());
	...
	private equalizeStorageAndTerminal(manager: Zerg): boolean {
		...
		if (
			(terminal.store.energy < equilibrium - tolerance || storage.energy > storageEnergyCap + storageTolerance) &&
			storage.energy > 0
		) {
		...
		if (terminal.store.energy > equilibrium + tolerance && storage.energy < storageEnergyCap) {
			if (this.unloadCarry(manager)) return true;
	...
	private moveMineralsToTerminal(manager: Zerg): boolean {
		...
		for (const resourceType in storage.store) {
			if (resourceType != RESOURCE_ENERGY && storage.store[resourceType as ResourceConstant] > 0) {
				if (this.unloadCarry(manager)) return true;
				manager.task = Tasks.withdraw(storage, resourceType as ResourceConstant);
				manager.task.parent = Tasks.transfer(terminal, resourceType as ResourceConstant);
				return true;
			}
		}
		return false;
