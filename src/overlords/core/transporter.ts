...
export class TransportOverlord extends Overlord {
  ...
  init () {
    ...
    const numTransporters = Math.ceil(
    ...
    if (request) {
        ...
        if (isResource(request.target)) {
          ...
          if (request.resourceType == 'all') {
            if (
              ...
              log.error(
                'TransportOverlord: ' + ALL_RESOURCE_TYPE_ERROR
              )
              return
            }
      ...
      if (transporter.carry.getUsedCapacity() > 0) {
        if (transporter.hasMineralsInCarry) {
          ...
          const dropoffPoints: Array<StructureLink | StructureStorage> =
          ...
          const bestDropoffPoint =
						transporter.pos.findClosestByMultiRoomRange(
              dropoffPoints
						)
  ...
  private handleBigTransporter (bigTransporter: Zerg) {
    const bestRequestViaStableMatching =
			this.colony.logisticsNetwork.matching[bigTransporter.name]
    ...
    if (tombstone) {
      const resourceType = _.last(
        _.sortBy(
          ...
          (resourceType) =>
            tombstone.store[(resourceType as ResourceConstant)] || 0
        )
      )
