...
import { ALL_RESOURCE_TYPE_ERROR, type BufferTarget, type LogisticsRequest } from "../../logistics/LogisticsNetwork";
...
export class TransportOverlord extends Overlord {
	...
	private neededTransportPower(): number {
		if (!this.colony.storage && !this.colony.hatchery?.battery && !this.colony.upgradeSite.battery) {
			return 0;
		}
	...
	init() {
		...
		const setup =
			this.colony.roomPlanner.roadPlanner.roadCoverage < ROAD_COVERAGE_THRESHOLD
	...
	private handleTransporter(transporter: Zerg, request: LogisticsRequest | undefined) {
		if (request) {
			...
			const bestChoice = _.last(
				_.sortBy(choices, choice => (request.multiplier * choice.dQ) / Math.max(choice.dt, 0.1))
			)!;
			let task = null;
			...
			if (transporter.carry.getUsedCapacity() > 0) {
				if (transporter.hasMineralsInCarry) {
					...
					const dropoffPoints: Array<StructureLink | StructureStorage> = _.compact([this.colony.storage!]);
	...
	private pickupDroppedResources(transporter: Zerg) {
		...
		if (tombstone) {
			const resourceType = _.last(
				_.sortBy(_.keys(tombstone.store), resourceType => tombstone.store[resourceType as ResourceConstant] || 0)
			);
			transporter.withdraw(tombstone, resourceType as ResourceConstant);
		}
	}
