...
import { type Hatchery } from "../../hiveClusters/hatchery";
import { type TransportRequest } from "../../logistics/TransportRequestGroup";
...
import { getStoreUsage, hasMinerals, mergeSum, minBy } from "../../utilities/utils";
...
export class BunkerQueenOverlord extends Overlord {
	...
	quadrants: Record<string, SupplyStructure[]>;
	...
	constructor(hatchery: Hatchery, priority = OverlordPriority.core.queen) {
		super(hatchery.colony, "supply", priority);
		this.room = hatchery.room;
	...
	private computeQueenAssignments() {
		...
		this.assignments = _.fromPairs(_.map(this.queens, queen => [queen.name, {}]));
		...
		if (this.numActiveQueens > 0) {
			...
			for (const quadrant of quadrantAssignmentOrder) {
				...
				_.extend(this.assignments[queen.name], _.fromPairs(_.map(quadrant, s => [s.id, true])));
				i++;
			}
		}
	}
	...
	private buildSupplyTaskManifest(queen: Zerg): Task | null {
		...
		const queenCarry = {} as Record<string, number>;
		const allStore = mergeSum(_.map(this.storeStructures, s => getStoreUsage(s.store)));
		...
		for (const request of supplyRequests) {
			...
			const remainingAmount = queen.carry.getUsedCapacity();
		...
		const targets: StoreStructure[] = _.filter(this.storeStructures, s =>
			_.every(neededResources, resource => (s.store[resource] || 0) >= (queenCarry[resource] || 0))
		);
	...
	private buildWithdrawTaskManifest(queen: Zerg): Task | null {
		...
		const queenCarry = { energy: 0 } as Record<string, number>;
		...
		for (const request of withdrawRequests) {
			// stop when carry will be full
			const remainingAmount = queen.carry.getFreeCapacity();
			if (remainingAmount == 0) break;
			// figure out how much you can withdraw
	...
	private handleQueen(queen: Zerg): void {
		...
		if (
			...
			_.some(_.keys(this.assignments[queen.name]), id => this.colony.transportRequests.withdrawByID[id])
		) {
		...
		// Does something need supplying?
			...
			_.some(_.keys(this.assignments[queen.name]), id => this.colony.transportRequests.supplyByID[id])
		) {
	...
	run() {
		this.autoRun(this.queens, queen => {
