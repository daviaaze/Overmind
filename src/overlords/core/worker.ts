...
import { type Colony, ColonyStage, DEFCON } from "../../Colony";
...
import { Overlord, type ZergOptions } from "../Overlord";
...
export class WorkerOverlord extends Overlord {
	...
	fortifyBarriers: Array<StructureWall | StructureRampart>;
	criticalBarriers: Array<StructureWall | StructureRampart>;
	constructionSites: ConstructionSite[];
	nukeDefenseRamparts: StructureRampart[];
	nukeDefenseHitsRemaining: Record<string, number>;
	...
	constructor(colony: Colony, priority = OverlordPriority.ownedRoom.work) {
		...
		this.criticalBarriers = $.structures(
			this,
			"criticalBarriers",
			() => _.filter(this.fortifyBarriers, barrier => barrier.hits < WorkerOverlord.settings.barrierHits.critical),
			10
		);
		...
		this.constructionSites = _.filter(this.colony.constructionSites, function (site) {
			...
			const structureAmount = room[(site.structureType + "s") as keyof Room]
				? (room[(site.structureType + "s") as keyof Room]! as Structure[]).length
				: room[site.structureType as keyof Room]
					? 1
					: 0;
			...
			if (defcon > DEFCON.safe) {
				...
				if (site.pos.roomName != homeRoomName && Cartographer.roomType(site.pos.roomName) == ROOMTYPE_CONTROLLER) {
					return site.structureType != STRUCTURE_CONTAINER && !(site.room && site.room.dangerousHostiles.length > 0);
				} else {
					return true;
				}
			}
		...
		if (this.colony.stage == ColonyStage.Larva) {
			numWorkers = $.number(this, "numWorkers", () => {
				...
				const energyMinedPerTick = _.sum(
					_.map(this.colony.miningSites, function (site) {
						...
						const miningPowerAssigned = _.sumBy(overlord.miners, miner => miner.getActiveBodyparts(WORK));
			...
			if (this.colony.roomPlanner.memory.relocating) {
				...
				numWorkers = $.number(this, "numWorkers", () => {
					...
					const buildTicks =
						_.sumBy(this.constructionSites, site => Math.max(site.progressTotal - site.progress, 0)) / BUILD_POWER;
					const repairTicks =
						_.sumBy(this.repairStructures, structure => structure.hitsMax - structure.hits) / REPAIR_POWER;
					const paveTicks = _.sumBy(this.colony.rooms, room => this.colony.roadLogistics.energyToRepave(room)) / 1; // repairCost=1
					let fortifyTicks = 0;
					if (this.colony.assets.energy > WorkerOverlord.settings.fortifyDutyThreshold) {
						fortifyTicks =
							(0.25 *
								_.sumBy(this.fortifyBarriers, barrier =>
					...
					let numWorkers = Math.ceil(
						(2 * (5 * buildTicks + repairTicks + paveTicks + fortifyTicks)) / (workPartsPerWorker * CREEP_LIFE_TIME)
					);
	...
	private buildPavingManifest(worker: Zerg, room: Room): Task | null {
		...
		const targetRefs: Record<string, boolean> = {};
		const tasks: Task[] = [];
		...
		while (energy > 0) {
			if (previousPos) {
				target = _.find(
					this.colony.roadLogistics.repairableRoads(room),
					road => road.hits < road.hitsMax && !targetRefs[road.id] && road.pos.getRangeTo(previousPos!) <= 1
				);
			} else {
				target = _.find(
					this.colony.roadLogistics.repairableRoads(room),
	...
	private fortifyActions(worker: Zerg, fortifyStructures = this.fortifyBarriers): boolean {
		let lowBarriers: Array<StructureWall | StructureRampart>;
		...
		if (highestBarrierHits > WorkerOverlord.settings.hitTolerance) {
			...
			lowBarriers = _.filter(
				fortifyStructures,
				structure => structure.hits <= lowestBarrierHits + WorkerOverlord.settings.hitTolerance
			);
		} else {
	...
	private handleWorker(worker: Zerg) {
		if (worker.carry.energy > 0) {
			...
			if (
				(this.colony.level < 8 || this.colony.upgradeSite.overlord.upgraders.length == 0) &&
