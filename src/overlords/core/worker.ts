...
export class WorkerOverlord extends Overlord {
  ...
  constructor (colony: Colony, priority = OverlordPriority.ownedRoom.work) {
    ...
    if (this.room.find(FIND_NUKES).length > 0) {
      for (const rampart of this.colony.room.ramparts) {
        ...
        if (rampart.hits < neededHits) {
          ...
          this.nukeDefenseHitsRemaining[rampart.id] =
    ...
    const approximateRepairPowerPerLifetime =
			((REPAIR_POWER * 50) / 3) * CREEP_LIFE_TIME
    if (
      totalNukeDefenseHitsRemaining >
			3 * approximateRepairPowerPerLifetime
    ) {
    ...
    if (this.colony.stage == ColonyStage.Larva) {
      numWorkers = $.number(this, 'numWorkers', () => {
        ...
        const energyMinedPerTick = _.sum(
          _.map(this.colony.miningSites, function (site) {
            ...
            const miningPowerAssigned = _.sumBy(
              overlord.miners,
      ...
      if (this.colony.roomPlanner.memory.relocating) {
        ...
        numWorkers = $.number(this, 'numWorkers', () => {
          ...
          const buildTicks =
						_.sumBy(this.constructionSites, (site) =>
          ...
          const paveTicks =
						_.sumBy(this.colony.rooms, (room) =>
              this.colony.roadLogistics.energyToRepave(room)
						) / 1 // repairCost=1
          let fortifyTicks = 0
          if (
  ...
  private handleWorker (worker: Zerg) {
    if (worker.carry.energy > 0) {
      ...
      // Acquire more energy
      const workerWithdrawLimit =
				this.colony.stage == ColonyStage.Larva ? 750 : 100
