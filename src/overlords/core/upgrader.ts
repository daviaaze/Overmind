...
export class UpgradingOverlord extends Overlord {
  ...
  constructor (
    ...
    super(upgradeSite.overlord.initializer, 'upgrade', priority)
  ...
  init () {
    ...
    if (
      this.colony.assets[RESOURCE_ENERGY] >
				UpgradeSite.settings.energyBuffer ||
			this.upgradeSite.controller.ticksToDowngrade < 500
    ) {
      const setup =
				this.colony.level == 8
          ? Setups.upgraders.rcl8
          : Setups.upgraders.default
      if (this.colony.level == 8) {
        this.wishlist(1, setup)
      } else {
        const upgradePowerEach = setup.getBodyPotential(
          WORK,
          this.colony
        )
        const upgradersNeeded = Math.ceil(
