...
export type RoomPlan = Record<string, {
  map: StructureMap
  pos: RoomPosition
  rotation: number
}>
...
export function getAllStructureCoordsFromLayout (
...
export class RoomPlanner {
  ...
  private generatePlan (level = 8): RoomPlan {
    ...
    for (const name in this.placements) {
      ...
      if (layout) {
        ...
        const pos =
					this.placements[
            (name as 'hatchery' | 'commandCenter' | 'bunker')
					]
        if (!pos) continue
  ...
  private mapFromPlan (plan: RoomPlan): StructureMap {
    ...
    const componentMaps: StructureMap[] = _.map(
      plan,
      (componentPlan) => componentPlan.map
    )
    const structureNames: string[] = _.uniq(
  ...
  private translateComponent (
    map: StructureMap,
    fromPos: RoomPosition | Coord,
    toPos: RoomPosition | Coord
  ): void {
  ...
  getObstacles (): RoomPosition[] {
    ...
    const passableStructureTypes: string[] = [
      STRUCTURE_ROAD,
      STRUCTURE_CONTAINER,
      STRUCTURE_RAMPART
    ]
  ...
  structureShouldBeHere (
    ...
    if (structureType == STRUCTURE_ROAD) {
      ...
      if (
        ...
        const maxRange = 4
        return (
          pos.findInRange(thingsBuildingLinksAndContainers, maxRange)
            .length > 0
        )
      }
    }
            ...
            if (!this.colony.storage) {
              ...
              this.colony.terminal &&
							this.colony.terminal.store.getUsedCapacity() -
								this.colony.terminal.store.energy >
								1000
            ) {
              log.info(
  ...
  private nextNeededLinkAnchor (): RoomPosition | undefined {
    ...
    const farthestSources = _.sortBy(
      this.colony.room.sources,
      (source) => -1 * Pathing.distance(origin, source.pos)
    )
  ...
  init (): void {
    if (this.active && getAutonomyLevel() == Autonomy.Automatic) {
      ...
      if (this.colony.spawns.length > 0) {
        ...
        const expansionData =
					this.colony.room.memory[_RM.EXPANSION_DATA]
        if (expansionData) {
          bunkerAnchor = derefCoords(
            expansionData.bunkerAnchor,
            this.colony.room.name
          )
        } else {
          log.error(
            'Cannot determine anchor! No spawns or expansionData.bunkerAnchor!'
          )
          return
        }
      }
  ...
  shouldRecheck (offset = 0): boolean {
    if (
      ...
      return (
        Game.time % (2 * RoomPlanner.settings.siteCheckFrequency) ==
				2 * this.colony.id + offset
      )
    } else {
      return (
        Game.time % RoomPlanner.settings.siteCheckFrequency ==
				2 * this.colony.id + offset
      )
    }
  }
