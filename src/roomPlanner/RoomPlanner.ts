...
export interface BuildingPlannerOutput {
	...
	shard: string;
	rcl: string;
	buildings: Record<string, { pos: Coord[] }>;
}
...
export interface StructureLayout {
	...
	data: {
		anchor: Coord;
		pointsOfInterest?: Record<string, Coord>;
	};
}

export type StructureMap = Record<string, RoomPosition[]>;

export type RoomPlan = Record<
	string,
	{
		map: StructureMap;
		pos: RoomPosition;
...
export interface PlannerMemory {
	...
	mapsByLevel?: Record<number, Record<string, ProtoPos[]>>;
	savedFlags: Array<{
...
export function getAllStructureCoordsFromLayout(layout: StructureLayout, rcl: number): Coord[] {
	...
	return _.uniqBy(coords, coord => coord.x + 50 * coord.y);
}
...
export class RoomPlanner {
	...
	private recallMap(level = this.colony.controller.level): void {
		if (this.memory.bunkerData?.anchor) {
	...
	private generatePlan(level = 8): RoomPlan {
		...
		for (const name in this.placements) {
			...
			if (layout) {
				...
				const pos = this.placements[name as "hatchery" | "commandCenter" | "bunker"];
				if (!pos) continue;
	...
	getStructureMapForBunkerAt(anchor: { x: number; y: number }, level = 8): StructureMap {
		...
		const structureLayout = _.mapValues(bunkerLayout[level]!.buildings, obj => obj.pos) as Record<string, Coord[]>;
				...
				for (const structureType in this.memory.mapsByLevel[8]) {
					if (!passableStructureTypes.includes(structureType)) {
						obstacles = obstacles.concat(
							_.map(this.memory.mapsByLevel[8][structureType], protoPos => derefRoomPosition(protoPos))
						);
					}
				}
			}
	...
	finalize(ignoreRoads = false): void {
		...
		const layoutIsValid: boolean =
			(!!this.placements.commandCenter && !!this.placements.hatchery) || !!this.placements.bunker;
		if (layoutIsValid) {
			...
			log.warning(
				"Not a valid room layout! Must have both hatchery and commandCenter placements " + "or bunker placement."
			);
		}
	}
		...
		if (structureType == STRUCTURE_ROAD) {
			...
			if (structureType == STRUCTURE_CONTAINER || structureType == STRUCTURE_LINK) {
				const thingsBuildingLinksAndContainers = _.map(
					[...this.colony.room.sources, this.colony.room.mineral!, this.colony.controller],
					thing => thing.pos
				);
				const maxRange = 4;
	...
	private demolishMisplacedStructures(skipRamparts = true, destroyAllStructureTypes = false): void {
		...
		if (this.colony.terminal) {
			if (
				(this.colony.storage && !this.structureShouldBeHere(STRUCTURE_STORAGE, this.colony.storage.pos)) ||
		...
		for (const priority of DemolishStructurePriorities) {
			...
			let removeCount = 0;
			let structures: Structure[] = _.filter(
			...
			for (const structure of structures) {
				if (
					...
					if (this.colony.level < 4 && (structureType == STRUCTURE_STORAGE || structureType == STRUCTURE_TERMINAL)) {
					...
					if (this.colony.level >= 6 && structureType == STRUCTURE_TERMINAL) {
						if (!this.colony.storage) {
							...
							this.colony.terminal.store.getUsedCapacity() - this.colony.terminal.store.energy > 1000
						) {
					...
					const amountMissing =
						CONTROLLER_STRUCTURES[structureType][this.colony.level] - structures.length + removeCount;
					if (amountMissing < maxRemoved) {
						if (structureType == STRUCTURE_SPAWN && this.colony.spawns.length == 1) {
							...
							const workTicksAvailable = _.sumBy(
								this.colony.overlords.work.workers,
								worker => worker.getActiveBodyparts(WORK) * (worker.ticksToLive || 0)
							);
					...
					if (count > 0 && RoomPlanner.canBuild(structureType, pos)) {
						...
						if (result != OK) {
							...
							for (const structure of structures) {
								...
								if (
									!this.structureShouldBeHere(structure.structureType, pos) &&
	...
	private nextNeededLinkAnchor(): RoomPosition | undefined {
		const linksEtAl = _.map(
			([] as Array<StructureLink | ConstructionSite>).concat(
				this.colony.links,
				_.filter(this.colony.constructionSites, site => site.structureType == STRUCTURE_LINK)
			),
			s => s.pos
		);
	...
	private buildNeededLinks() {
		...
		if (
			...
			(this.colony.bunker || this.colony.hatchery?.link) &&
	...
	visuals(): void {
		...
		if (getAutonomyLevel() < Autonomy.Automatic) {
			...
			if (expansionData) {
				...
				if (bunkerPos) {
					Visualizer.drawLayout(bunkerLayout, bunkerPos, {
						opacity: 0.2
					});
				}
			}
