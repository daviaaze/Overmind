...
export interface RoadPlannerMemory {
	roadLookup: Record<string, Record<string, boolean>>;
	roadCoverage: number;
	roadCoverages: Record<
		string,
		{
			roadCount: number;
			length: number;
...
export class RoadPlanner {
	...
	private computeRoadCoverage(
		...
		if (_.every(roomNames, roomName => Game.rooms[roomName] || $.costMatrixRecall(roomName, MatrixTypes.default))) {
			...
			return { roadCount, length: path.length };
		}
		return
	}
	...
	finalize(): void {
		...
		if (_.keys(this.roomPlanner.map).length > 0) {
			...
			if (this.roomPlanner.memory.bunkerData?.anchor) {
				...
				roomPlannerRoads = _.map(this.roomPlanner.memory.mapsByLevel[8][STRUCTURE_ROAD], protoPos =>
