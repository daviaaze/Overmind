...
import { getAllStructureCoordsFromLayout, type StructureLayout } from "../RoomPlanner";
...
const _allBunkerCoords: Record<number, Coord[]> = {};
...
export const bunkerCoordLookup = _.mapValues(_allBunkerCoords, (coordArr: Coord[]) =>
	_.fromPairs(_.map(coordArr, c => [coordName(c), true]))
) as Record<number, Record<string, true | undefined>>;
...
export function insideBunkerBounds(pos: RoomPosition, colony: Colony): boolean {
	if (colony.roomPlanner.memory.bunkerData?.anchor) {
...
export const bunkerChargingSpots: Coord[] = [
	{ x: 29, y: 24 },
	{ x: 24, y: 21 }
];
...
export const energyStructureOrder: Coord[] = ([] as Coord[]).concat(
