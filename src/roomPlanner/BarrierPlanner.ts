...
export interface BarrierPlannerMemory {
  barrierLookup: Record<string, boolean | undefined>
}
...
export class BarrierPlanner {
  ...
  finalize (): void {
    ...
    if (this.barrierPositions.length == 0) {
      if (this.roomPlanner.bunkerPos) {
        ...
        log.error(
					`Couldn't generate barrier plan for ${this.colony.name}!`
        )
      }
    }
  ...
  barrierShouldBeHere (pos: RoomPosition): boolean {
    if (this.colony.layout == 'bunker') {
      if (this.colony.level >= BarrierPlanner.settings.bunkerizeRCL) {
        ...
        return (
          !!this.memory.barrierLookup[pos.coordName] ||
					pos.getRangeTo(this.colony.controller) == 1
        )
      }
    } else {
  ...
  private buildMissingRamparts (): void {
    ...
    const criticalStructures: Structure[] = _.compact([
      ...this.colony.towers,
      ...this.colony.spawns,
      this.colony.storage!,
    ...
    if (this.roomPlanner.active) {
      ...
      if (
        !this.roomPlanner.memory.relocating &&
				this.colony.level >=
					BarrierPlanner.settings.buildBarriersAtRCL &&
