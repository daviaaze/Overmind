...
export class BasePlanner {
	static getBunkerLocation(room: Room, visualize = true): RoomPosition | undefined {
		...
		const minimizePathLengthTo: RoomPosition[] = _.map(_.compact([...room.sources, room.controller]), obj => obj.pos);
	...
	private static getAllowableBunkerLocations(room: Room, visualize = true): RoomPosition[] {
		...
		allowableLocations = _.filter(
			allowableLocations,
			anchor => !_.some(sitesAndMineral, pos => this.bunkerIntersectsWith(anchor, pos, 1))
		);
		if (visualize) {
