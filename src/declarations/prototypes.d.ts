...
type Sink = StructureSpawn | StructureExtension | StructureLab | StructurePowerSpawn | StructureNuker | StructureTower;
...
interface RoomPosition {
	...
	findClosestByLimitedRange: <T>(objects: T[] | RoomPosition[], rangeLimit: number) => T | undefined;
...
interface RoomVisual {
	...
	structure: (x: number, y: number, type: string, opts?: Record<string, number>) => RoomVisual;

	connectRoads: (opts?: Record<string, number>) => RoomVisual | void;

	speech: (text: string, x: number, y: number, opts?: Record<string, number>) => RoomVisual;
