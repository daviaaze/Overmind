...
import { type MoveOptions, type SwarmMoveOptions } from './Movement'
...
export class Pathing {
  ...
  static findPath (
    ...
    if (ret.incomplete && options.ensurePath) {
      if (options.useFindRoute == undefined) {
        ...
        if (roomDistance <= 2) {
          log.warning(
						`Movement: path failed without findroute. Origin: ${origin.print}, ` +
          ...
          log.warning(
						`Movement: second attempt was ${ret.incomplete ? 'not ' : ''}successful`
          )
          return ret
        }
      } else {
        // TODO: handle case where pathfinder failed at a long distance even with findRoute
      }
    }
    return ret
  }
  ...
  static findSwarmPath (
    ...
    return PathFinder.search(
      origin,
      _.map(destinations, (pos) => ({ pos, range: options.range! })),
      {
        maxOps: options.maxOps,
        maxRooms: options.maxRooms,
        plainCost: 1,
  ...
  static roomCallback (
    ...
    origin: RoomPosition,
    destination: RoomPosition,
    allowedRooms: Record<string, boolean> | undefined,
    options: MoveOptions
  ): CostMatrix | boolean {
  ...
  static swarmRoomCallback (
    ...
    if (room && !options.ignoreStructures) {
      matrix = this.getSwarmDefaultMatrix(
        room,
        width,
        height,
        options,
      ...
      matrix = this.getSwarmTerrainMatrix(
        roomName,
        width,
        height,
        options.exitCost
  ...
  static findKitingPath (
    ...
    fleeFrom: Array<RoomPosition | HasPos>,
    options: MoveOptions = {}
  ): PathFinderPath {
    _.defaults(options, {
      fleeRange: 5,
    ...
    const avoidGoals = _.map(fleeFromPos, (pos) => {
      return { pos, range: options.fleeRange! }
    })
  ...
  private static getCostMatrixForInvisibleRoom (
    roomName: string,
    options: MoveOptions,
    clone = true
  ): CostMatrix | boolean {
  ...
  static getTerrainMatrix (
    roomName: string,
    costs: TerrainCosts = { plainCost: 1, swampCost: 5 }
  ): CostMatrix {
    return $.costMatrix(
      roomName,
			`terrain:${costs.plainCost}:${costs.swampCost}`,
			() => {
  ...
  static getSwarmTerrainMatrix (
    ...
    const matrix = $.costMatrix(
      roomName,
			`swarmTerrain${width}x${height}EC${exitCost}`,
			() => {
  ...
  /**
	 ...
	 */
  static blockMyCreeps (
    matrix: CostMatrix,
    room: Room,
    creeps?: Array<Creep | Zerg>
  ) {
    const blockCreeps = creeps || (room.creeps as Array<Creep | Zerg>)
    const blockPositions = _.map(blockCreeps, (creep) =>
      Overmind.zerg[creep.name]
        ? Overmind.zerg[creep.name].nextPos
        : creep.pos
    )
    ...
    for (let dx = -range; dx <= range; dx++) {
      ...
      for (let dy = -range; dy <= range; dy++) {
        ...
        matrix.set(
          x,
          y,
          add ? Math.min(cost + currentCost, 200) : cost
        )
      }
    }
  }
  ...
  static findRoute (
    ...
    const restrictDistance =
			options.restrictDistance || linearDistance + 10
    ...
    if (options.preferHighway) {
      ...
      // 	if (_.some([oCoords.x, oCoords.y, dCoords.x, dCoords.y], z => z % 10 <= 1 || z % 10 >= 9)) {
      // 		highwayBias = 2.5;
      // 	}
      // }
    }
    ...
    if (!_.isArray(ret)) {
      log.warning(
				`Movement: couldn't findRoute from ${origin} to ${destination}!`
      )
    } else {
  ...
  static calculatePathWeight (
    ...
    for (const pos of ret.path) {
      if (!pos.room) {
        // If you don't have vision, assume there are roads
        weight += 1
      } else {
  ...
  static weightedDistance (arg1: RoomPosition, arg2: RoomPosition): number {
    ...
    if (arg1.name < arg2.name) {
      // alphabetize since path lengths are the same either direction
      pos1 = arg1
      pos2 = arg2
    } else {
      pos1 = arg2
    ...
    if (!Memory.pathing.weightedDistances[pos1.name][pos2.name]) {
      Memory.pathing.weightedDistances[pos1.name][pos2.name] =
				this.calculatePathWeight(pos1, pos2)
    }
  ...
  static isReachable (
    startPos: RoomPosition,
    endPos: RoomPosition,
    obstacles: Array<RoomPosition | HasPos>,
    options: MoveOptions = {}
  ): boolean {
    _.defaults(options, {
      ignoreCreeps: true,
