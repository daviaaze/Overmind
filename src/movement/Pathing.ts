...
export class Pathing {
	 ...
	 */
	static findShortestPath(startPos: RoomPosition, endPos: RoomPosition, options: MoveOptions = {}): PathFinderPath {
		_.defaults(options, {
			ignoreCreeps: true,
			range: 1,
			direct: true
	 ...
	 */
	static blockMyCreeps(matrix: CostMatrix, room: Room, creeps?: Array<Creep | Zerg>) {
		const blockCreeps = creeps || (room.creeps as Array<Creep | Zerg>);
		const blockPositions = _.map(blockCreeps, creep =>
			Overmind.zerg[creep.name] ? Overmind.zerg[creep.name].nextPos : creep.pos
		);
	...
	static findRoute(
		...
		const ret = Game.map.findRoute(origin, destination, {
			routeCallback: (roomName: string) => {
				...
				if (!options.allowHostile && this.shouldAvoid(roomName) && roomName !== destination && roomName !== origin) {
