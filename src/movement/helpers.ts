...
export function getCreepWeightInfo(creep: Creep, analyzeCarry = true): { move: number; weighted: number } {
	...
	const bodyParts = _.countBy(creep.body, p => (_.find(unweightedParts, p.type) ? p.type : "weighted"));
	bodyParts.move = bodyParts.move || 0;
	bodyParts.weighted = bodyParts.weighted || 0;
	if (bodyParts[CARRY]) {
		bodyParts.weighted += Math.ceil(creep.store.getUsedCapacity() / CARRY_CAPACITY);
	}
	// Account for boosts
	for (const part of creep.body) {
		if (part.type == MOVE && part.boost) {
			bodyParts.move += BOOSTS.move[part.boost as "ZO" | "ZHO2" | "XZHO2"].fatigue - 1;
		}
	}
	return bodyParts as {
		move: number;
 ...
 */
export function getTerrainCosts(creep: Creep): {
	plainCost: number;
	swampCost: number;
} {
