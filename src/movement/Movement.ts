...
export class Movement {
	...
	static goTo(creep: Zerg, destination: HasPos | RoomPosition, options: MoveOptions = {}): number {
		...
		if (!moveData.path || moveData.path.length == 0) {
			...
			if (ret.incomplete) {
				...
				log.debug(
					`Movement: incomplete path for ${creep.print}! ` + `(${creep.pos.print} ${rightArrow} ${destination.print})`
				);
				color = "red";
			}
	...
	private static getDestination(
		...
		const nextWaypoint = _.find(
			waypoints,
			waypoint => !_.some(waypointsVisited, visited => waypoint.isEqualTo(visited))
		);
	...
	static vacatePos(pos: RoomPosition, suicide = false): boolean {
		...
		const nearbyCreeps = _.compact(
			_.map(pos.findInRange(FIND_MY_CREEPS, 2), creep => Overmind.zerg[creep.name])
		) as Zerg[];
		...
		if (moved) {
			...
			if (suicide) {
				log.debug(
					`Could not move creep ${blockingCreep.name} off of ${blockingCreep.pos.print}! ` + "Suiciding creep! (RIP)"
				);
	...
	static swarmCombatMove(
		...
		const callback = (roomName: string) => {
			...
			if (room) {
				...
				const otherCreeps = _.filter(room.creeps, creep => !_.some(swarm.creeps, c => c.name == creep.name));
		...
		if (approach.length > 0) {
			if (!_.some(approach, goal => swarm.minRangeTo(goal) <= goal.range)) {
	...
	static combatMove(
		...
		// Flee from bad things that that you're too close to
		if (avoid.length > 0) {
			if (_.some(avoid, goal => creep.pos.inRangeToXY(goal.pos.x, goal.pos.y, goal.range)) && !creep.inRampart) {
		...
		// Approach things you want to go to if you're out of range of all the baddies
		if (approach.length > 0) {
			if (!_.some(approach, goal => creep.pos.inRangeToXY(goal.pos.x, goal.pos.y, goal.range))) {
		...
		if ((options.preferRamparts || options.requireRamparts) && !creep.inRampart && approach.length > 0) {
			const openRamparts = _.filter(
				...
				rampart => _.some(approach, g => rampart.pos.inRangeToXY(g.pos.x, g.pos.y, g.range)) && rampart.pos.isWalkable()
			);
	 ...
	 */
	static kite(creep: Zerg, avoidGoals: Array<RoomPosition | HasPos>, options: MoveOptions = {}): number | undefined {
		_.defaults(options, {
			fleeRange: 5,
