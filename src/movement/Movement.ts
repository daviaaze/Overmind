...
import { normalizeZerg, type Zerg } from '../zerg/Zerg'
...
export interface MoveOptions {
  ...
  route?: Record<string, boolean> // lookup table for allowable pathing rooms
...
export class Movement {
  ...
  static goTo (
    ...
    if (creep.fatigue > 0) {
      Movement.circle(creep.pos, 'aqua', 0.3)
      return ERR_TIRED
    }
    ...
    if (
      options.range != undefined &&
			destination.rangeToEdge <= options.range
    ) {
    ...
    if (options.range != undefined && rangeToDestination <= options.range) {
      if (destination.isEqualTo(finalDestination)) {
        ...
        // debug
        console.log(
					`Destination ${destination} not equal to final destination ${finalDestination}!`
        )
    ...
    if (options.waypoints && !destination.isEqualTo(finalDestination)) {
      const portalTraversed = this.traversePortalWaypoint(
        creep,
        destination
      )
      if (portalTraversed) {
    ...
    if (!destination.isEqualTo(state.destination)) {
      if (
        options.movingTarget &&
				state.destination.isNearTo(destination)
      ) {
        if(!moveData.path) {
          moveData.path = ''
        }
    ...
    if (
      options.repathOnceVisible &&
			!(moveData.roomVisibility || {})[creep.room.name]
    ) {
      delete moveData.path
    }
  ...
  private static getDestination (
    ...
    const waypointsVisited = _.compact(
      _.map(moveData.waypointsVisited || [], (posName) =>
        getPosFromString(posName)
      )
    )
    const nextWaypoint = _.find(
      waypoints,
      (waypoint) =>
        !_.some(waypointsVisited, (visited) =>
          waypoint.isEqualTo(visited)
        )
    )
  ...
  /**
	 ...
	 */
  private static traversePortalWaypoint (
    creep: Zerg,
    portalPos: RoomPosition
  ): boolean {
    if (
      creep.pos.roomName == portalPos.roomName &&
			creep.pos.getRangeTo(portalPos) > 1
    ) {
      log.error(
        'Movement.travelPortalWaypoint() should only be called in range 1 of portal!'
      )
    }
  ...
  private static crossPortal (creep: Zerg, portalPos: RoomPosition): boolean {
    if (
      ...
      const creepOnPortal =
				!!creep.pos.lookForStructure(STRUCTURE_PORTAL)
      if (!creepOnPortal) {
        return true
      } else {
      ...
      if (creep.pos.getRangeTo(portalPos) > 1) {
        log.error(
          'Movement.crossPortal() should only be called in range 1 of portal!'
        )
      } else {
    ...
    if (this.getPushPriority(pusher) < this.getPushPriority(pushee)) {
      ...
      if (isZerg(pushee)) {
        ...
        if (pushee.task && pushee.task.isWorking) {
          ...
          return (
            _.filter(
              pushee.pos.availableNeighbors().concat(pusher.pos),
  ...
  private static getPushDirection (
    pusher: Zerg | Creep,
    pushee: Zerg | Creep
  ): DirectionConstant {
  ...
  static pushCreep (creep: Zerg, otherCreep: Creep | Zerg): boolean {
    ...
    const otherNextPos =
			otherCreep.pos.getPositionAtDirection(pushDirection)
    if (isZerg(otherCreep)) {
      if (outcome == OK) {
        if (otherData?.path && !otherCreep.blockMovement) {
  ...
  static recursivePush (
    creep: Zerg,
    excludePos: RoomPosition[] = []
  ): RoomPosition | undefined {
    const creepPos = creep.pos
    let movePos: RoomPosition | undefined = _.find(
      creepPos.availableNeighbors(),
      (neighbor) => !_.some(excludePos, (pos) => pos.isEqualTo(neighbor))
    )
    if (movePos) {
      ...
      const availablePositions = _.filter(
        ...
        (neighbor) =>
          !_.some(excludePos, (pos) => pos.isEqualTo(neighbor))
      )
  ...
  static swarmMove (
    ...
    if (swarm.fatigue > 0) {
      Movement.circle(swarm.anchor, 'aqua', 0.3)
      console.log('tired')
      return ERR_TIRED
    }
    ...
    if (
      options.range != undefined &&
			swarm.minRangeTo(destination) <= options.range &&
			swarm.maxRangeTo(destination) <=
				options.range + Math.max(swarm.width, swarm.height)
    ) {
    ...
    if (!moveData.path) {
      ...
      if (Game.time % 10 == 0 && state.cpu > REPORT_SWARM_CPU_THRESHOLD) {
        log.alert(
					`Movement: heavy cpu use for swarm with ${_.first(swarm.creeps)!.print}, cpu: ${state.cpu}. ` +
      ...
      if (ret.incomplete) {
        log.debug(
					`Movement: incomplete path for swarm with ${_.first(swarm.creeps)!.print}! ` +
						`(${swarm.anchor.print} ${rightArrow} ${destination.print})`
        )
        color = 'red'
      }
  ...
  private static combatMoveCallbackModifier (
    room: Room,
    matrix: CostMatrix,
  ...
  static swarmCombatMove (
    ...
    const callback = (roomName: string) => {
      ...
      if (room) {
        matrix = Pathing.getSwarmDefaultMatrix(
          room,
          swarm.width,
          swarm.height
        ) // already cloned
        ...
        matrix = Pathing.getSwarmTerrainMatrix(
          roomName,
          swarm.width,
          swarm.height
        )
    ...
    if (avoid.length > 0) {
      if (_.some(avoid, (goal) => swarm.minRangeTo(goal) <= goal.range)) {
    ...
    if (approach.length > 0) {
      if (
        !_.some(
          approach,
          (goal) => swarm.minRangeTo(goal) <= goal.range
        )
      ) {
  ...
  static combatMove (
    ...
    const callback = (roomName: string) => {
      if (roomName == creep.room.name) {
        ...
        return !(
          Memory.rooms[roomName] && Memory.rooms[roomName][_RM.AVOID]
        )
      }
    }
    ...
    if (
      (options.preferRamparts || options.requireRamparts) &&
			!creep.inRampart &&
			approach.length > 0
    ) {
  ...
  static kite (
    creep: Zerg,
    avoidGoals: Array<RoomPosition | HasPos>,
    options: MoveOptions = {}
  ): number | undefined {
    _.defaults(options, {
      fleeRange: 5,
    ...
    const nextPos = _.first(
      Pathing.findKitingPath(creep.pos, avoidGoals, options).path
    )
    if (nextPos) {
  ...
  static flee (
    ...
    if (rangeToClosest > options.fleeRange) {
  ...
  /**
	 ...
	 */
  private static updateStateNextCoord (
    moveData: MoveData,
