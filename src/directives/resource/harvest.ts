...
export interface DirectiveHarvestMemory extends FlagMemory {
  ...
  constructor (flag: Flag) {
    ...
    if (this.colony) {
      ...
      this.colony.destinations.push({
        pos: this.pos,
        order: this.memory[_MEM.TICK] || Game.time
      })
    }
  ...
  get distance (): number {
    if (
      !this.memory[_HARVEST_MEM_PATHING] ||
			Game.time >= this.memory[_HARVEST_MEM_PATHING][_MEM.EXPIRATION]
    ) {
