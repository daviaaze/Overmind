...
export class DirectiveNukeResponse extends Directive {
  ...
  init (): void {
    if (this.nuke) {
      this.alert(
				`Nuclear impact in ${this.nuke.timeToLand}`,
				NotifierPriority.Critical
      )
    } else {
      this.alert(
        'Nuke response directive active!',
        NotifierPriority.Critical
      )
    }
  }
    ...
    if (Game.time % 50 == 0) {
      if (this.nuke) {
        const rampartPositions = _.filter(
          this.nuke.pos.getPositionsInRange(2),
          function (pos) {
        ...
        log.alert(
					`Incoming nuke at ${this.nuke.pos.print}! Time until impact: ${this.nuke.timeToLand}`
        )
      } else {
