...
export class DirectiveTerminalEmergencyState extends Directive {
  ...
  refresh () {
    ...
    this.terminal = this.pos.lookForStructure(
      STRUCTURE_TERMINAL
    ) as StructureTerminal
    if (this.terminal) {
      Overmind.terminalNetwork.registerTerminalState(
        this.terminal,
        TERMINAL_STATE_EMERGENCY
      )
    }
    if (Game.time % 25 == 0) {
      log.alert(
				`${this.pos.print}: emergency terminal state active!`,
  ...
  run () {
    ...
    if (
      !this.colony ||
			!this.terminal ||
			Game.time > (this.memory[_MEM.TICK] || 0) + EMERGENCY_STATE_TIMEOUT
    ) {
      this.remove()
    }
  }
