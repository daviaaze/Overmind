...
export class DirectiveInvasionDefense extends Directive {
  ...
  private readonly relocateFrequency: number
  ...
  spawnMoarOverlords () {
    ...
    if (this.colony.stage > ColonyStage.Larva) {
      this.overlords.rangedDefense = new RangedDefenseOverlord(
        this,
        useBoosts
      )
    } else {
      this.overlords.meleeDefense = new MeleeDefenseOverlord(
        this,
        useBoosts
      )
    }
    ...
    if (meleeHostiles.length > 0 && expectedDamage > ATTACK_POWER * 70) {
      this.overlords.bunkerDefense = new BunkerDefenseOverlord(
        this,
        false
      )
    }
