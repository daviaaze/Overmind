...
export class DirectiveInvasionDefense extends Directive {
	...
	spawnMoarOverlords() {
		...
		const useBoosts =
			...
			(expectedHealing > RANGED_ATTACK_POWER * 100 && !!this.colony.terminal && !!this.colony.evolutionChamber);
		// const percentWalls =
		// 	_.filter(
		//     this.room.barriers,
		...
		const meleeHostiles = _.filter(
			this.room.hostiles,
			hostile => hostile.getActiveBodyparts(ATTACK) > 0 || hostile.getActiveBodyparts(WORK) > 0
		);
