	...
	constructor(flag: Flag, colonyFilter?: (colony: Colony) => boolean) {
		...
		if (!colony) {
			if (Overmind.exceptions.length == 0) {
				...
				log.alert(
					`Could not get colony for directive ${this.print}; ` + "exceptions present this tick, so won't remove"
				);
			}
			return;
		}
				...
				if (room) {
					return (
						_.filter(
							room.flags,
							flag =>
								this.filter(flag) && !(flag.memory.setPosition && flag.memory.setPosition.roomName != pos.roomName)
						).length > 0
					);
				} else {
				...
				if (room) {
					return (
						_.filter(
							...
							flag => this.filter(flag) && !(flag.memory.setPosition && !equalXYR(pos, flag.memory.setPosition))
						).length > 0
					);
				} else {
