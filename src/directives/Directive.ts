...
export abstract class Directive {
  ...
  overlords: Record<string, Overlord> // Overlords
  ...
  constructor (flag: Flag, colonyFilter?: (colony: Colony) => boolean) {
    ...
    if (this.memory.waypoints) {
      this.waypoints = _.map(
        this.memory.waypoints,
        (posName) => getPosFromString(posName)!
      )
    }
    ...
    if (
      this.memory[_MEM.EXPIRATION] &&
			Game.time > this.memory[_MEM.EXPIRATION]
    ) {
    ...
    if (this.memory.setPosition) {
      ...
      if (!this.flag.pos.isEqualTo(pos)) {
        ...
        if (result == OK) {
          ...
          log.warning(
						`Could not set room position to ${JSON.stringify(this.memory.setPosition)}!`
          )
        }
      } else {
  ...
  private getColony (
    colonyFilter?: (colony: Colony) => boolean,
    verbose = false
  ): Colony | undefined {
    ...
    if (this.memory[_MEM.COLONY]) {
      ...
      const colony = Overmind.colonies[
        Overmind.colonyMap[this.pos.roomName]
      ] as Colony | undefined
      if (colony) {
  ...
  private findNearestColony (
    ...
    for (const colony of getAllColonies()) {
      if (
        Game.map.getRoomLinearDistance(this.pos.roomName, colony.name) >
				maxLinearRange
      ) {
        continue
      }
    ...
    switch (scope) {
      case 'room':
        if (room) {
              ...
              (flag) =>
                this.filter(flag) &&
                  ...
                  flag.memory.setPosition &&
									flag.memory.setPosition.roomName !=
										pos.roomName
								)
            ).length > 0
          )
          ...
          return (
            _.filter(flagsInRoom, (flag) => this.filter(flag))
              .length > 0
          )
        }
      case 'pos':
        if (room) {
          ...
          return (
            _.filter(flagsAtPos, (flag) => this.filter(flag))
              .length > 0
          )
        }
    }
    ...
    switch (scope) {
      case 'room':
        if (room) {
          ...
          if (opts.memory?.[_MEM.COLONY]) {
            createAtPos = Pathing.findPathablePosition(
              opts.memory[_MEM.COLONY]
            )
          } else {
            createAtPos = Pathing.findPathablePosition(
