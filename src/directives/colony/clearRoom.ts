...
export class DirectiveClearRoom extends Directive {
	...
	constructor(flag: Flag) {
		...
		if (Cartographer.roomType(this.pos.roomName) != ROOMTYPE_CONTROLLER) {
			log.warning(
				`${this.print}: ${printRoomName(this.pos.roomName)} is not a controller room; ` + "removing directive!"
			);
			this.remove(true);
		}
	}
	...
	private removeAllStructures(): boolean {
		const keepStorageStructures =
			this.memory.keepStorageStructures !== undefined ? this.memory.keepStorageStructures : true;
		...
		if (this.room) {
			...
			for (const s of allStructures) {
				...
				if (keepStorageStructures && (s.structureType == STRUCTURE_STORAGE || s.structureType == STRUCTURE_TERMINAL)) {
					continue;
				}
