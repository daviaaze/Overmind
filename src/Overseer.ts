...
interface OverseerMemory {
  suspendUntil: Record<string, number> // overlords are suspended until tick
}
...
export class Overseer implements IOverseer {
  ...
  private readonly overlords: Overlord[] // Overlords sorted by priority
  private sorted: boolean
  private overlordsByColony: Record<string, Overlord[]> // Overlords grouped by colony
  private readonly directives: Directive[] // Directives across the colony
  ...
  private try (callback: () => void, identifier?: string): void {
    if (USE_TRY_CATCH) {
      try {
        callback()
  ...
  private handleOutpostDefense (colony: Colony) {
    ...
    for (const room of colony.outposts) {
      ...
      if (Cartographer.roomType(room.name) != ROOMTYPE_SOURCEKEEPER) {
        ...
        const defenseFlags = _.filter(
          room.flags,
          (flag) =>
            DirectiveGuard.filter(flag) ||
						DirectiveOutpostDefense.filter(flag)
        )
        if (
          room.dangerousHostiles.length > 0 &&
					defenseFlags.length == 0
        ) {
  ...
  private handleColonyInvasions (colony: Colony) {
    ...
    if (colony.room) {
      ...
      const effectiveInvaderCount = _.sum(
        _.map(colony.room.hostiles, (invader) =>
          invader.boosts.length > 0 ? 2 : 1
        )
      )
      const needsDefending =
				effectiveInvaderCount >= 3 ||
  ...
  private computePossibleOutposts (colony: Colony, depth = 3): string[] {
    return _.filter(
      ...
      (roomName) => {
        ...
        const alreadyAnOutpost = _.some(
          Overmind.cache.outpostFlags,
          (flag) =>
            (flag.memory.setPosition || flag.pos).roomName ==
						roomName
        )
        ...
        const isReachableFromColony = _.some(neighboringRooms, (r) =>
  ...
  private handleNewOutposts (colony: Colony) {
    const numSources = _.sumBy(colony.roomNames, (roomName) =>
      Memory.rooms[roomName]?.[_RM.SOURCES]
    ...
    if (numRemotes < Colony.settings.remoteSourcesByLevel[colony.level]) {
      ...
      const bestOutpost = minBy(possibleOutposts, function (roomName) {
        ...
        const sourceCoords = Memory.rooms[roomName][_RM.SOURCES]
        if (!sourceCoords) return false
        const sourcePositions = _.map(sourceCoords, (src) =>
          derefCoords(src.c, roomName)
        )
        const sourceDistances = _.map(sourcePositions, (pos) =>
          Pathing.distance(origin, pos)
        )
        if (
          _.some(
            sourceDistances,
      ...
      if (bestOutpost) {
        ...
        DirectiveOutpost.createIfNotPresent(pos, 'room', {
          memory: { [_MEM.COLONY]: colony.name }
        })
      }
    }
  }
  ...
  getCreepReport (colony: Colony): string[][] {
    ...
    const roleOccupancy: Record<string, [number, number]> = {}
    ...
    for (const overlord of this.overlordsByColony[colony.name]) {
      for (const role in overlord.creepUsageReport) {
        ...
        if (report == undefined) {
          ...
          if (spoopyBugFix) {
            // bizzarely, if you comment these lines out, the creep report is incorrect
            log.debug(`report: ${JSON.stringify(report)}`)
            log.debug(
							`occupancy: ${JSON.stringify(roleOccupancy)}`
            )
          }
        }
      }
