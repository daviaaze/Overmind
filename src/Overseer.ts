...
export class Overseer implements IOverseer {
	...
	private registerLogisticsRequests(colony: Colony): void {
		...
		for (const tombstone of colony.tombstones) {
			if (
				tombstone.store.getUsedCapacity() > LogisticsNetwork.settings.droppedEnergyThreshold ||
		...
		for (const room of colony.outposts) {
			...
			if (Cartographer.roomType(room.name) != ROOMTYPE_SOURCEKEEPER) {
				...
				const defenseFlags = _.filter(
					room.flags,
					flag => DirectiveGuard.filter(flag) || DirectiveOutpostDefense.filter(flag)
				);
	...
	private handleNewOutposts(colony: Colony) {
		const numSources = _.sumBy(colony.roomNames, roomName =>
			Memory.rooms[roomName]?.[_RM.SOURCES] ? Memory.rooms[roomName][_RM.SOURCES]!.length : 0
		);
	...
	private handleSafeMode(colony: Colony): void {
		...
		const criticalStructures = _.compact([...colony.spawns, colony.storage, colony.terminal]) as Structure[];
