...
export class Abathur {
	...
	private canReceiveBasicMineralsForReaction(
		...
		for (const mineral in mineralQuantities) {
			if (!this.someColonyHasExcess(mineral as ResourceConstant, mineralQuantities[mineral])) {
				return false;
			}
		}
		return true;
	...
	private canBuyBasicMineralsForReaction(mineralQuantities: Record<string, number>): boolean {
		...
		for (const mineral in mineralQuantities) {
			...
			if (Overmind.tradeNetwork.priceOf(mineral as ResourceConstant) > maxPrice) {
				return false;
			}
		}
		return true;
	 ...
	 */
	getMissingBasicMinerals(reactionQueue: Reaction[], verbose = false): Record<string, number> {
	...
	private ingredientsList(mineral: ResourceConstant): ResourceConstant[] {
		if (!REAGENTS[mineral] || _.isEmpty(mineral)) {
			...
			return this.ingredientsList(REAGENTS[mineral][0]).concat(this.ingredientsList(REAGENTS[mineral][1]), mineral);
		}
	}
}
