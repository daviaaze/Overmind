...
export const priorityStockAmounts: Record<string, number> = {
...
export const wantedStockAmounts: Record<string, number> = {
  UH: 3000, // (+100 % attack)
...
export const baseStockAmounts: Record<string, number> = {
...
export class Abathur {
  ...
  private _globalAssets: Record<string, number>
  ...
  /**
	 ...
	 */
  private computeGlobalAssets (): Record<string, number> {
    const colonyAssets: Array<Record<string, number>> = []
  ...
  get globalAssets (): Record<string, number> {
  ...
  private canReceiveBasicMineralsForReaction (
    mineralQuantities: Record<string, number>,
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    amount: number
  ): boolean {
    for (const mineral in mineralQuantities) {
      if (
        !this.someColonyHasExcess(
  ...
  private canBuyBasicMineralsForReaction (mineralQuantities: Record<string, number>): boolean {
    ...
    for (const mineral in mineralQuantities) {
      ...
      if (
        Overmind.tradeNetwork.priceOf((mineral as ResourceConstant)) >
				maxPrice
      ) {
        return false
      }
  ...
  private hasExcess (
    ...
    return (
      this.assets[mineralType] - excessAmount >
  ...
  private someColonyHasExcess (
    ...
    return _.some(getAllColonies(), (colony) =>
      colony.abathur.hasExcess(mineralType, excessAmount)
    )
  }
      ...
      for (const resourceType in stocks) {
        ...
        if (amountOwned < amountNeeded) {
          ...
          if (
            ...
            if (verbose) {
              console.log(
								`Missing minerals for ${resourceType}: ${JSON.stringify(missingBaseMinerals)}`
              )
            }
          }
        }
  ...
  private buildReactionQueue (
    ...
    if (verbose) {
      console.log(
				`Abathur@${this.colony.room.print}: building reaction queue for ${amount} ${mineral}`
      )
    }
    ...
    for (const ingredient of this.ingredientsList(mineral)) {
      ...
      if (ingredient != mineral) {
        if (verbose) {
          console.log(
						`productionAmount: ${productionAmount}, assets: ${this.assets[ingredient]}`
          )
        }
        productionAmount = Math.max(
          productionAmount - (this.assets[ingredient] || 0),
          0
        )
      }
      ...
      for (const index of [precursor1, precursor2]) {
        if (index != -1) {
          if (reactionQueue[index].amount == 0) {
            ...
            reactionQueue[index].amount = minMax(
              reaction.amount,
              Abathur.settings.minBatchSize,
  ...
  getMissingBasicMinerals (
    ...
    const requiredBasicMinerals =
			this.getRequiredBasicMinerals(reactionQueue)
    if (verbose) {
      console.log(
				`Required basic minerals: ${JSON.stringify(requiredBasicMinerals)}`
      )
    }
    if (verbose) console.log(`assets: ${JSON.stringify(this.assets)}`)
    const missingBasicMinerals: Record<string, number> = {}
    for (const mineralType in requiredBasicMinerals) {
      const amountMissing =
				requiredBasicMinerals[mineralType] -
				(this.assets[mineralType] || 0)
      if (amountMissing > 0) {
    ...
    if (verbose) {
      console.log(
				`Missing basic minerals: ${JSON.stringify(missingBasicMinerals)}`
      )
    }
  ...
  /**
	 ...
	 */
  private getRequiredBasicMinerals (reactionQueue: Reaction[]): Record<string, number> {
    const requiredBasicMinerals: Record<string, number> = {
      [RESOURCE_HYDROGEN]: 0,
      [RESOURCE_OXYGEN]: 0,
      [RESOURCE_UTRIUM]: 0,
      [RESOURCE_KEANIUM]: 0,
  ...
  private ingredientsList (mineral: ResourceConstant): ResourceConstant[] {
    if (!REAGENTS[mineral] || _.isEmpty(mineral)) {
      ...
      return this.ingredientsList(REAGENTS[mineral][0]).concat(
        this.ingredientsList(REAGENTS[mineral][1]),
        mineral
      )
    }
  }
