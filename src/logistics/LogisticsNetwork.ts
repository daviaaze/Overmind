...
export class LogisticsNetwork {
	...
	colony: Colony;
	private targetToRequest: Record<string, number>;
	private _matching: Record<string, LogisticsRequest | undefined> | undefined;

	// private logisticPositions: { [roomName: string]: RoomPosition[] };
	private cache: {
		nextAvailability: Record<string, [number, RoomPosition]>;
		predictedTransporterCarry: Record<string, StoreDefinition>;
		resourceChangeRate: Record<string, Record<string, number>>;
	};
	...
	requestOutputMinerals(target: StoreStructure, opts = {} as RequestOptions): void {
		for (const resourceType in target.store) {
			...
			if (amount > 0) {
				opts.resourceType = resourceType as ResourceConstant;
			...
			if (approximateDistance) {
				for (const targetPos of targetPositions.slice(1)) {
					...
					approximateDistance += Math.ceil(
						pos.getMultiRoomRangeTo(targetPos) * LogisticsNetwork.settings.rangeToPathHeuristic
					);
	...
	private computePredictedTransporterCarry(
		...
		if (transporter.task && transporter.task.target) {
			...
			if (requestID) {
				...
				if (request) {
					const remainingCapacity = transporter.carry.getFreeCapacity();
					...
					if (request.resourceType == "all") {
						...
						for (const resourceType in request.target.store) {
							const resourceFraction =
								(request.target.store[resourceType as ResourceConstant] || 0) / request.target.store.getUsedCapacity();
							if (transporter.carry[resourceType as ResourceConstant]) {
								transporter.carry[resourceType as ResourceConstant] += resourceAmount * resourceFraction;
								transporter.carry[resourceType as ResourceConstant] = minMax(
									transporter.carry[resourceType as ResourceConstant],
									0,
									remainingCapacity
								);
							} else {
								transporter.carry[resourceType as ResourceConstant] = minMax(resourceAmount, 0, remainingCapacity);
							}
						}
					} else {
						if (transporter.carry[request.resourceType]) {
							transporter.carry[request.resourceType] += resourceAmount;
							transporter.carry[request.resourceType] = minMax(
								transporter.carry[request.resourceType],
								0,
								remainingCapacity
							);
						} else {
							transporter.carry[request.resourceType] = minMax(resourceAmount, 0, remainingCapacity);
						}
					}
					return transporter.carry as StoreDefinition;
				}
			}
		}
	...
	predictedRequestAmount(
		...
		const eta =
			busyUntil + LogisticsNetwork.settings.rangeToPathHeuristic * newPos.getMultiRoomRangeTo(request.target.pos);
		...
		if (request.amount > 0) {
			...
			if (isStoreStructure(request.target)) {
				// cap predicted amount at storeCapacity
			...
			const resourceInflux = _.sum(
				_.map(otherTargetingTransporters, other => other.carry[request.resourceType as ResourceConstant] || 0)
			);
			...
			let predictedAmount = request.amount + predictedDifference;
			if (isStoreStructure(request.target)) {
				// cap predicted amount at -1 * storeCapacity
			...
			const resourceOutflux = _.sum(_.map(otherTargetingTransporters, other => other.carry.getFreeCapacity()));
	...
	bufferChoices(
		...
		const choices: Array<{ dQ: number; dt: number; targetRef: string }> = [];
		...
		if (amount > 0) {
			...
			const dt_direct =
				ticksUntilFree +
				newPos.getMultiRoomRangeTo(request.target.pos) * LogisticsNetwork.settings.rangeToPathHeuristic;
			choices.push({
				dQ: dQ_direct,
				dt: dt_direct,
				targetRef: request.target.ref
			});
			if ((carry[request.resourceType] || 0) > amount || carry.getUsedCapacity() == transporter.carryCapacity) {
			...
			for (const buffer of this.buffers) {
				...
				const dt_buffer =
					newPos.getMultiRoomRangeTo(buffer.pos) * LogisticsNetwork.settings.rangeToPathHeuristic +
			...
			const remainingCarryCapacity = transporter.carry.getFreeCapacity();
			...
			for (const buffer of this.buffers) {
				const dQ_buffer = Math.min(Math.abs(amount), transporter.carryCapacity, buffer.store.getFreeCapacity());
				const dt_buffer =
					newPos.getMultiRoomRangeTo(buffer.pos) * LogisticsNetwork.settings.rangeToPathHeuristic +
					Pathing.distance(buffer.pos, request.target.pos) +
					ticksUntilFree;
	...
	summarize(): void {
		...
		for (const transporter of this.colony.overlords.logistics.transporters) {
			...
			const target = transporter.task
				? transporter.task.proto._target.ref + " " + transporter.task.targetPos.printPlain
				: "none";
	...
	get matching(): Record<string, LogisticsRequest | undefined> {
		if (!this._matching) {
	 ...
	 */
	private stableMatching(transporters: Zerg[]): Record<string, LogisticsRequest | undefined> {
		const tPrefs: Record<string, string[]> = {};
		for (const transporter of transporters) {
			tPrefs[transporter.name] = _.map(this.transporterPreferences(transporter), request => request.id);
		}
		const rPrefs: Record<string, string[]> = {};
		for (const request of this.requests) {
