...
interface LogisticsNetworkMemory {
  transporterCache: Record<string, {
...
export class LogisticsNetwork {
  ...
  private _matching:
    | Record<string, LogisticsRequest | undefined>
    | undefined
  ...
  constructor(colony: Colony) {
    this.memory = Mem.wrap(
      colony.memory,
      'logisticsNetwork',
      LogisticsNetworkMemoryDefaults
    )
    this.requests = []
  ...
  requestOutput(target: LogisticsTarget, opts = {} as RequestOptions): void {
    ...
    if (
      ...
      (isStoreStructure(target) || isTombstone(target))
    ) {
      ...
      if (requestID) {
        ...
        if (request) {
          ...
          if (request.resourceType == 'all') {
            if (
              !isStoreStructure(request.target) &&
            ...
            for (const resourceType in request.target.store) {
              const resourceFraction =
                ...
                ] || 0) / request.target.store.getUsedCapacity()
  ...
  predictedRequestAmount(
    ...
    if (request.amount > 0) {
      ...
      if (isStoreStructure(request.target)) {
        ...
        predictedAmount = Math.min(
          predictedAmount,
          request.target.storeCapacity
        )
      } else if (isEnergyStructure(request.target)) {
        predictedAmount = Math.min(
          predictedAmount,
          request.target.energyCapacity
        )
      }
      const resourceInflux = _.sum(
        _.map(
          ...
          (other) =>
            other.carry[(request.resourceType as ResourceConstant)] ||
            0
        )
      )
      ...
      if (isStoreStructure(request.target)) {
        ...
        predictedAmount = Math.max(
          predictedAmount,
          -1 * request.target.storeCapacity
        )
      } else if (isEnergyStructure(request.target)) {
        predictedAmount = Math.min(
          predictedAmount,
          -1 * request.target.energyCapacity
        )
      }
      const resourceOutflux = _.sum(
        _.map(
          otherTargetingTransporters,
          (other) => other.carry.getFreeCapacity()
        )
      )
  ...
  bufferChoices(
    ...
    if (amount > 0) {
      ...
      if (
        ...
        carry.getUsedCapacity() == transporter.carryCapacity
      ) {
      ...
      const remainingCarryCapacity =
        transporter.carry.getFreeCapacity()
      const dQ_direct = Math.min(
        Math.abs(amount),
