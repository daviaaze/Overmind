...
export class TransportRequestGroup {
	supply: Record<number, TransportRequest[]>;
	withdraw: Record<number, TransportRequest[]>;
	supplyByID: Record<string, TransportRequest[]>;
	withdrawByID: Record<string, TransportRequest[]>;
	...
	summarize(ignoreEnergy = false): void {
		...
		for (const priority in this.supply) {
			...
			for (const request of this.supply[priority]) {
				...
				console.log(
					`    targetID: ${request.target.ref}  amount: ${request.amount}  ` + `resourceType: ${request.resourceType}`
				);
			}
		}
