...
import { type Hatchery, type SpawnRequest } from '../hiveClusters/hatchery'
...
export class SpawnGroup {
  ...
  constructor (
    ...
    this.memory = Mem.wrap(
      Memory.rooms[this.roomName],
      'spawnGroup',
      SpawnGroupMemoryDefaults
    )
    this.ref = initializer.ref + ':SG'
    this.stats = {
      avgDistance:
				_.sum(Object.values(this.memory.distances)) /
    ...
    if (this.colonyNames.length == 0) {
      log.warning(
				`No colonies meet the requirements for SwarmGroup: ${this.ref}`
      )
    }
  ...
  private recalculateColonies () {
    ...
    const colonyRoomsInRange = _.filter(
      getAllColonyRooms(),
      (room) =>
        Game.map.getRoomLinearDistance(room.name, this.roomName) <=
				MAX_LINEAR_DISTANCE
    )
    const colonies = [] as string[]
    const routes = {} as Record<string, Record<string, boolean>>
    ...
    for (const colonyRoom of colonyRoomsInRange) {
      ...
      if (spawn) {
        ...
        if (
          route &&
					!path.incomplete &&
					path.path.length <= MAX_PATH_DISTANCE
        ) {
  ...
  init (): void {
    ...
    for (const request of this.requests) {
      const maxCost = bodyCost(
        request.setup.generateBody(this.energyCapacityAvailable)
      )
      ...
      const bestHatchery = minBy(
        okHatcheries,
        (hatchery) => hatchery.nextAvailability + distanceTo(hatchery)
      )
      if (bestHatchery) {
        bestHatchery.enqueue(request)
      } else {
        log.warning(
					`Could not enqueue creep ${request.setup.role} in ${this.roomName}, ` +
