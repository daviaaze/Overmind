...
export class SpawnGroup {
	...
	constructor(initializer: SpawnGroupInitializer, settings: Partial<SpawnGroupSettings> = {}) {
		...
		this.stats = {
			avgDistance: _.sum(Object.values(this.memory.distances)) / _.keys(this.memory.distances).length || 100
		};
		this.requests = [];
		...
		this.colonyNames = _.filter(
			...
			roomName =>
				this.memory.distances[roomName] <= this.settings.maxPathDistance &&
				Game.rooms[roomName] &&
		...
		this.energyCapacityAvailable = _.max(
			_.map(this.colonyNames, roomName => Game.rooms[roomName].energyCapacityAvailable)
		)!;
	...
	private recalculateColonies() {
		...
		const colonyRoomsInRange = _.filter(
			getAllColonyRooms(),
			room => Game.map.getRoomLinearDistance(room.name, this.roomName) <= MAX_LINEAR_DISTANCE
		);
