...
export class RoadLogistics {
  ...
  private _assignedWorkers: Record<string, string[]>
  ...
  private workerShouldRepaveRoom (worker: Zerg, room: Room): boolean {
    ...
    if (
      otherAssignedWorkers.length <
			RoadLogistics.settings.allowedPaversPerRoom
    ) {
      if (this.assignedWorkers(room).includes(worker.name)) {
        ...
        return (
          this.criticalRoads(room).length > 0 ||
					this.energyToRepave(room) >= worker.carryCapacity
        )
      }
    } else {
      return false
        ...
        _.sortBy(
          _.filter(
            ...
            (road) =>
              road.hits <
								road.hitsMax *
									RoadLogistics.settings.criticalThreshold &&
							this.colony.roomPlanner.roadShouldBeHere(road.pos)
          ),
          (road) => road.pos.getMultiRoomRangeTo(this.colony.pos)
        ),
      ROAD_CACHE_TIMEOUT
    )
  }
        ...
        _.sortBy(
          _.filter(
            ...
            (road) =>
              road.hits <
								road.hitsMax *
									RoadLogistics.settings.repairThreshold &&
							this.colony.roomPlanner.roadShouldBeHere(road.pos)
          ),
          (road) => road.pos.getMultiRoomRangeTo(this.colony.pos)
        ),
