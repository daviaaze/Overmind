...
import { type Colony, getAllColonies } from '../Colony'
...
interface TerminalNetworkStats {
  transfers: {
    [resourceType: string]: Record<string, Record<string, number>>
...
export class TerminalNetwork implements ITerminalNetwork {
  ...
  exceptionTerminals: Record<string, TerminalState> // Terminals in a special operational state
  assets: Record<string, number> // All assets
  ...
  constructor (terminals: StructureTerminal[]) {
    ...
    this.memory = Mem.wrap(
      Memory.Overmind,
      'terminalNetwork',
      TerminalNetworkMemoryDefaults
    )
    this.stats = Mem.wrap(
      Memory.stats.persistent,
      'terminalNetwork',
      TerminalNetworkStatsDefaults,
      true
    )
    ...
    this.notifications = []
    this.averageFullness =
			_.sumBy(
			  this.terminals,
			  (t) => t.store.getUsedCapacity() / t.store.getCapacity()
  ...
  private getAllAssets (): Record<string, number> {
    return mergeSum(
      _.map(this.terminals, (terminal) => colonyOf(terminal).assets)
    )
  }
  ...
  private transfer (
    ...
    if (response == OK) {
      let msg =
				`${sender.room.print} ${rightArrow} ${amount} ${resourceType} ${rightArrow} ` +
				`${receiver.room.print} `
      if (description) {
        msg += `(for ${description})`
      }
      ...
      _.remove(
        this.readyTerminals,
        (terminal) => terminal.id == sender.id
      )
    } else {
      log.warning(
  ...
  requestResource (
    ...
    const possibleSenders = _.filter(
      this.terminals,
      (terminal) =>
        (terminal.store[resourceType] || 0) > amount + minDifference &&
				terminal.cooldown == 0 &&
				!this.alreadySent.includes(terminal) &&
				terminal.id != receiver.id
    )
    const sender: StructureTerminal | undefined = maxBy(
      possibleSenders,
      (t) => t.store[resourceType] || 0
    )
    if (sender) {
      this.transfer(
        sender,
  ...
  private handleExcess (terminal: StructureTerminal, threshold = 25000): void {
    const terminalNearCapacity =
			terminal.store.getUsedCapacity() >
			0.95 * terminal.store.getCapacity()
    ...
    for (const resource in terminal.store) {
      ...
      if (resource == RESOURCE_ENERGY) {
        let energyThreshold =
					Energetics.settings.terminal.energy.outThreshold
        if (terminalNearCapacity) {
          // if you're close to full, be more agressive with selling energy
          energyThreshold =
        ...
        if (terminal.store[RESOURCE_ENERGY] > energyThreshold) {
          ...
          const storageEnergyCap =
						Energetics.settings.storage.total.cap
          if (!storage || storage.energy >= storageEnergyCap) {
            if (terminalNearCapacity) {
              // just get rid of stuff at high capacities
              const response = Overmind.tradeNetwork.sellDirectly(
                terminal,
                RESOURCE_ENERGY,
                amount,
                true
              ...
              const response = Overmind.tradeNetwork.sell(
                terminal,
                RESOURCE_ENERGY,
                amount,
                MAX_ENERGY_SELL_ORDERS
        ...
        if (terminal.store[(resource as ResourceConstant)] > threshold) {
          const receiver = maxBy(this.terminals, (terminal) =>
            wantedAmount(
              colonyOf(terminal),
          ...
          if (
            ...
            // Sell excess
            if (
              terminalNearCapacity ||
							terminal.store[(resource as ResourceConstant)] >
								2 * threshold
            ) {
              const response = Overmind.tradeNetwork.sellDirectly(
                terminal,
  ...
  private equalize (
    ...
    const maxSendSize =
			resourceType == RESOURCE_ENERGY
			  ? TerminalNetwork.settings.equalize.maxEnergySendSize
			  : TerminalNetwork.settings.equalize.maxMineralSendSize
    const averageAmount =
			_.sum(
			  _.map(
			    terminals,
			    (terminal) => colonyOf(terminal).assets[resourceType] || 0
			  )
			) / terminals.length

    const terminalsByResource = _.sortBy(
      terminals,
      (terminal) => colonyOf(terminal).assets[resourceType] || 0
    )
    if (verbose) {
      log.debug(
        _.map(
          terminalsByResource,
          (t) =>
						`${t.room.name}: ${colonyOf(t).assets[resourceType]}`
        )
      )
    }
    // Min-max match terminals
    const receivers = _.take(
      terminalsByResource,
      Math.floor(terminalsByResource.length / 2)
    )
    terminalsByResource.reverse()

    const senders = _.take(
      terminalsByResource,
      Math.floor(terminalsByResource.length / 2)
    )
    ...
    for (const [sender, receiver] of _.zip(senders, receivers)) {
      if (verbose) {
        log.debug(
					` > ${sender!.room.print} to ${receiver!.room.print}...`
        )
      }
      const senderAmount = colonyOf(sender!).assets[resourceType] || 0
      const receiverAmount =
				colonyOf(receiver!).assets[resourceType] || 0
      const tolerance =
				TerminalNetwork.settings.equalize.tolerance[resourceType] ||
				TerminalNetwork.settings.equalize.tolerance.default
      if (verbose) {
        log.debug(
					`    sender amt: ${senderAmount}  receiver amt: ${receiverAmount}  tolerance: ${tolerance}`
        )
      }
      if (
        senderAmount - receiverAmount < tolerance &&
      ...
      sendAmount = Math.min(
        sendAmount,
        (sender!.store[resourceType] || 0) - sendCost - 10,
        receiver!.store.getCapacity() -
					receiver!.store.getUsedCapacity()
      )
  ...
  private equalizeCycle (): void {
    ...
    const resource = equalizeResources[this.memory.equalizeIndex] as
			| ResourceConstant
			| undefined
    const terminals =
			resource == RESOURCE_POWER
			  ? _.filter(
			    this.terminals,
			    (t) => colonyOf(t).powerSpawn != undefined
			  )
			  : this.terminals
    if (resource) {
      // resource is undefined if there is nothing to equalize
      this.equalize(resource, terminals)
    }
    // Determine next resource type to equalize; most recent resourceType gets cycled to end
    const resourceEqualizeOrder = equalizeResources
      .slice(this.memory.equalizeIndex + 1)
    ...
    const nextResourceType = _.find(resourceEqualizeOrder, (resource) => {
      ...
      const amounts = _.map(
        this.terminals,
        (terminal) => colonyOf(terminal).assets[resource] || 0
      )
      const tolerance =
				TerminalNetwork.settings.equalize.tolerance[resource] ||
				TerminalNetwork.settings.equalize.tolerance.default
      return _.max(amounts)! - _.min(amounts)! > tolerance
    })
    // Set next equalize resource index
    this.memory.equalizeIndex = _.findIndex(
      equalizeResources,
      (resource) => resource == nextResourceType
    )
  }
  ...
  private handleTerminalState (
    ...
    for (const resourceType of RESOURCE_IMPORTANCE) {
      ...
      const tolerance =
				targetAmount == 0 ? TERMINAL_MIN_SEND : state.tolerance
      ...
      if (state.type == 'in' || state.type == 'in/out') {
        if (amount < targetAmount - tolerance) {
          ...
          if (sender) {
            const receiveAmount = minMax(
              targetAmount - amount,
              TERMINAL_MIN_SEND,
              maxSendSize
            )
            if (
              (sender.store[resourceType] || 0) >
							TERMINAL_MIN_SEND
            ) {
              this.transfer(
                sender,
      ...
      if (state.type == 'out' || state.type == 'in/out') {
        if (
          terminal.cooldown == 0 &&
					amount > targetAmount + tolerance
        ) {
          const receiver = minBy(this.terminals, (t) =>
  ...
  run (): void {
    ...
    for (const terminalID in this.exceptionTerminals) {
      this.handleTerminalState(
        deref(terminalID) as StructureTerminal,
        this.exceptionTerminals[terminalID]
      )
    }
    // Equalize resources
    ...
    // }
      ...
      const terminalToSellExcess =
				this.terminals[Game.time % this.terminals.length]
      ...
      if (Game.market.credits > TraderJoe.settings.market.energyCredits) {
        const averageEnergy =
					_.sumBy(
					  this.terminals,
					  (terminal) =>
					    colonyOf(terminal).assets[RESOURCE_ENERGY] || 0
					) / this.terminals.length
        if (
          averageEnergy < TerminalNetwork.settings.buyEnergyThreshold
        ) {
          const poorestTerminal = minBy(
            this.terminals,
            (terminal) =>
              colonyOf(terminal).assets[RESOURCE_ENERGY] || 0
          )
          if (poorestTerminal) {
            const amount =
							Energetics.settings.terminal.energy.tradeAmount
            Overmind.tradeNetwork.maintainBuyOrder(
              poorestTerminal,
              RESOURCE_ENERGY,
              amount,
              MAX_ENERGY_BUY_ORDERS
