...
export class TerminalNetwork implements ITerminalNetwork {
	...
	constructor(terminals: StructureTerminal[]) {
		...
		this.averageFullness =
			_.sumBy(this.terminals, t => t.store.getUsedCapacity() / t.store.getCapacity()) / this.terminals.length;
	}
	...
	private transfer(
		...
		if (response == OK) {
			let msg =
				`${sender.room.print} ${rightArrow} ${amount} ${resourceType} ${rightArrow} ` + `${receiver.room.print} `;
			if (description) {
				msg += `(for ${description})`;
			}
			this.notify(msg);
		...
		for (const resource in terminal.store) {
			...
			if (resource == RESOURCE_ENERGY) {
				...
				if (terminalNearCapacity) {
					...
					energyThreshold =
						Energetics.settings.terminal.energy.equilibrium + Energetics.settings.terminal.energy.tolerance;
				}
				...
				if (terminal.store[resource as ResourceConstant] > threshold) {
					const receiver = maxBy(this.terminals, terminal =>
						wantedAmount(colonyOf(terminal), resource as ResourceConstant)
					);
					if (receiver && wantedAmount(colonyOf(receiver), resource as ResourceConstant) > TERMINAL_MIN_SEND) {
						// Try to send internally first
						const response = this.transfer(terminal, receiver, resource as ResourceConstant, 1000, "excess resources");
						if (response == OK) return;
					} else {
						// Sell excess
						if (terminalNearCapacity || terminal.store[resource as ResourceConstant] > 2 * threshold) {
							const response = Overmind.tradeNetwork.sellDirectly(terminal, resource as ResourceConstant, 1000);
							if (response == OK) return;
						} else {
							const response = Overmind.tradeNetwork.sell(terminal, resource as ResourceConstant, 10000);
							if (response == OK) return;
						}
					}
				}
	...
	private equalize(resourceType: ResourceConstant, terminals = this.terminals, verbose = false): void {
		...
		const averageAmount =
			_.sum(_.map(terminals, terminal => colonyOf(terminal).assets[resourceType] || 0)) / terminals.length;
		...
		for (const [sender, receiver] of _.zip(senders, receivers)) {
			...
			sendAmount = Math.min(
				...
				receiver!.store.getCapacity() - receiver!.store.getUsedCapacity()
			);
	...
	private equalizeCycle(): void {
		...
		const terminals =
			resource == RESOURCE_POWER ? _.filter(this.terminals, t => colonyOf(t).powerSpawn != undefined) : this.terminals;
		if (resource) {
		...
		const nextResourceType = _.find(resourceEqualizeOrder, resource => {
			...
			const tolerance =
				TerminalNetwork.settings.equalize.tolerance[resource] || TerminalNetwork.settings.equalize.tolerance.default;
	...
	run(): void {
		...
		// }
			...
			if (Game.market.credits > TraderJoe.settings.market.energyCredits) {
				const averageEnergy =
					_.sumBy(this.terminals, terminal => colonyOf(terminal).assets[RESOURCE_ENERGY] || 0) / this.terminals.length;
