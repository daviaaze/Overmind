...
export class TraderJoe implements ITradeNetwork {
	...
	private buildMarketCache(verbose = false, orderThreshold = 1000): void {
		...
		const allOrders = Game.market.getAllOrders(
			order => !myActiveOrderIDs.includes(order.id) && order.amount >= orderThreshold
	...
	buy(terminal: StructureTerminal, resource: ResourceConstant, amount: number): void {
		...
		if (terminal.store[RESOURCE_ENERGY] < 10000 || terminal.store.getFreeCapacity() < amount) {
			return;
		}
		let ordersForMineral = Game.market.getAllOrders({
			resourceType: resource,
	...
	sellDirectly(
		...
		if (order) {
			...
			if (terminal.store[RESOURCE_ENERGY] > cost) {
				...
				this.logTransaction(order, terminal.room.name, amount ?? TERMINAL_MIN_SEND, response);
				return response;
			}
		}
	}
	...
	maintainBuyOrder(
		...
		const order = _.find(
			Game.market.orders,
			o => o.type == ORDER_BUY && o.resourceType == resource && o.roomName == terminal.room.name
		);
		if (order) {
			...
			if (order.remainingAmount < 2000) {
				...
				this.notify(
					`${terminal.room.print}: extending buy order for ${resource} by ${addAmount}.` + ` Response: ${ret}`
				);
			}
		} else {
			...
			if (ordersOfType.length < maxOrdersOfType) {
				...
				this.notify(
					`${terminal.room.print}: creating buy order for ${resource} at price ${marketHigh}. ` + `Response: ${ret}`
				);
			}
			// else {
	...
	private maintainSellOrder(
		...
		const order = _.find(
			Game.market.orders,
			o => o.type == ORDER_SELL && o.resourceType == resource && o.roomName == terminal.room.name
		);
		if (order) {
			...
			if (order.remainingAmount < 2000) {
				...
				const ret = Game.market.extendOrder(order.id, addAmount);
				this.notify(
					`${terminal.room.print}: extending sell order for ${resource} by ${addAmount}.` + ` Response: ${ret}`
				);
			}
		} else {
			...
			if (ordersOfType.length < maxOrdersOfType) {
				...
				this.notify(
					`${terminal.room.print}: creating sell order for ${resource} at price ${marketLow}. ` + `Response: ${ret}`
				);
			}
			// else {
