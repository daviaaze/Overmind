...
interface MarketCache {
  sell: Record<string, { high: number, low: number }>
  buy: Record<string, { high: number, low: number }>
  tick: number
}
...
export const maxMarketPrices: Record<string, number> = {
  default: 5.0,
...
export class TraderJoe implements ITradeNetwork {
  ...
  private buildMarketCache(verbose = false, orderThreshold = 1000): void {
    ...
    const myActiveOrderIDs = _.map(
      _.filter(Game.market.orders, (order) => order.active ?? false),
      (order) => order.id
    )
    ...
    for (const resourceType in groupedBuyOrders) {
      ...
      this.memory.cache.buy[resourceType] = { high, low }
    }
    for (const resourceType in groupedSellOrders) {
      ...
      this.memory.cache.sell[resourceType] = { high, low }
    }
  ...
  private cleanUpInactiveOrders() {
    ...
    const ordersToClean = _.filter(
      Game.market.orders,
      (o) =>
        (o.type == ORDER_SELL &&
          o.active == false &&
          o.remainingAmount == 0) || // if order is expired, or
        (Game.time - o.created >
          TraderJoe.settings.market.orders.timeout && // order is old and almost done
          o.remainingAmount <
          TraderJoe.settings.market.orders.cleanupAmount) ||
        (o.roomName && !Overmind.colonies[o.roomName]) ||
        false
    ) // order placed from dead colony
  ...
  lookForGoodDeals(
    ...
    const marketLow = this.memory.cache.sell[resource]
      ? this.memory.cache.sell[resource].low
      : undefined
    if (marketLow == undefined) {
      return
    }
    const order = maxBy(ordersForMineral, (order) =>
      this.effectiveBuyPrice(order, terminal)
    )
  ...
  buy(
    ...
    if (
      Game.market.credits < TraderJoe.settings.market.reserveCredits ||
      terminal.cooldown > 0
    ) {
      return
    }
    ...
    if (
      ...
      terminal.store.getFreeCapacity() < amount
    ) {
      return
    }
    let ordersForMineral = Game.market.getAllOrders({
    ...
    const bestOrder = minBy(
      ordersForMineral,
      (order: Order) => order.price
    )
  ...
  sellDirectly(
    ...
    const minAmount = flexibleAmount ? TERMINAL_MIN_SEND : amount ?? TERMINAL_MIN_SEND
    let ordersForMineral = Game.market.getAllOrders({
      resourceType: resource,
      type: ORDER_BUY
    })
    ...
    if (order) {
      const sellAmount = Math.min(order.amount, amount ?? TERMINAL_MIN_SEND)
      const cost = Game.market.calcTransactionCost(
        sellAmount,
        terminal.room.name,
        order.roomName!
  ...
  maintainBuyOrder(
    ...
    const marketHigh = this.memory.cache.buy[resource]
      ? this.memory.cache.buy[resource].high
      : undefined
    if (!marketHigh) {
      return
    }
    ...
    if (order) {
      if (
        order.price < marketHigh ||
        (order.price > marketHigh && order.remainingAmount == 0)
      ) {
        const ret = Game.market.changeOrderPrice(order.id, marketHigh)
        this.notify(
          `${terminal.room.print}: updating buy order price for ${resource} from ` +
      ...
      if (order.remainingAmount < 2000) {
        ...
        this.notify(
          `${terminal.room.print}: extending buy order for ${resource} by ${addAmount}.` +
          ` Response: ${ret}`
        )
      }
    } else {
      const ordersOfType = _.filter(
        Game.market.orders,
      ...
      if (ordersOfType.length < maxOrdersOfType) {
        ...
        this.notify(
          `${terminal.room.print}: creating buy order for ${resource} at price ${marketHigh}. ` +
          `Response: ${ret}`
        )
      }
      // else {
  ...
  private maintainSellOrder(
    ...
    if (order) {
      if (
        order.price > marketLow ||
        (order.price < marketLow && order.remainingAmount == 0)
      ) {
        const ret = Game.market.changeOrderPrice(order.id, marketLow)
        this.notify(
          `${terminal.room.print}: updating sell order price for ${resource} from ` +
      ...
      if (order.remainingAmount < 2000) {
        ...
        const ret = Game.market.extendOrder(order.id, addAmount)
        this.notify(
          `${terminal.room.print}: extending sell order for ${resource} by ${addAmount}.` +
          ` Response: ${ret}`
        )
      }
    } else {
      ...
      if (ordersOfType.length < maxOrdersOfType) {
        ...
        this.notify(
          `${terminal.room.print}: creating sell order for ${resource} at price ${marketLow}. ` +
          `Response: ${ret}`
        )
      }
      // else {
  ...
  private logTransaction(
    ...
    //   )
    //   : 0
    const roomName = Game.rooms[terminalRoomName]
      ? Game.rooms[terminalRoomName].print
      : terminalRoomName
    let msg: string
    if (order.type == ORDER_SELL) {
      msg =
        `${roomName} ${leftArrow} ${amount} ${order.resourceType} ${leftArrow} ` +
        `${printRoomName(order.roomName!)} (result: ${response})`
    } else {
      msg =
        `${roomName} ${rightArrow} ${amount} ${order.resourceType} ${rightArrow} ` +
        `${printRoomName(order.roomName!)} (result: ${response})`
    }
    this.notify(msg)
  }
      ...
      if (transaction.time < time) {
        ...
        if (transaction.order) {
          ...
          if (!this.stats.bought[resourceType]) {
            this.stats.bought[resourceType] = {
              amount: 0,
              credits: 0
            }
          }
