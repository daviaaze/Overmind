...
export function verifyLaunchManifest () {
  for (const i in launchFrom) {
    ...
    const nuker: StructureNuker =
			Overmind.colonies[from].commandCenter.nuker

    if (Game.map.getRoomLinearDistance(from, to) > NUKE_RANGE) {
      log.info(`${from} to ${to} is out of range!`)
    } else if (
      !nuker ||
			nuker.ghodium < nuker.ghodiumCapacity ||
			nuker.energy < nuker.energyCapacity
    ) {
...
export function doomsdayLaunch () {
  for (const i in launchFrom) {
    ...
    if (nuker.cooldown == 0) {
      ...
      log.alert(
				`[NUCLEAR LAUNCH] Launching nuke from ${from} to ${pos.print}! Result: ${ret}`
      )
    }
  }
}
