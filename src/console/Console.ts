...
import { type Colony, type ColonyMemory } from "../Colony";
...
export class OvermindConsole {
	...
	static setSignature(signature: string | undefined): string | undefined {
		const sig = signature || DEFAULT_OVERMIND_SIGNATURE;
		if (sig.length > 100) {
	...
	static removeAllLogisticsDirectives(): string {
		const logisticsFlags = _.filter(
			Game.flags,
			flag => flag.color == COLOR_YELLOW && flag.secondaryColor == COLOR_YELLOW
		);
	...
	static listPersistentDirectives(): string {
		...
		for (const i in Overmind.directives) {
			...
			if (dir.memory.persistent) {
				msg += `Type: ${dir.directiveName}`.padRight(20) + `Name: ${dir.name}`.padRight(15) + `Pos: ${dir.pos.print}\n`;
			}
		}
		return msg;
	}
	...
	static deepCleanMemory(): string {
		...
		for (const i in Game.creeps) {
			if (Object.keys(Game.creeps[i].memory).length === 0) {
		...
		for (const i in Memory.flags) {
			if ((Memory.flags[i] as any).overlords) {
				delete (Memory.flags[i] as any).overlords;
			}
		}
		// Clean creep memory
	...
	private static recursiveMemoryProfile(memoryObject: any, sizes: RecursiveObject, currentDepth: number): void {
		for (const key in memoryObject) {
			if (currentDepth == 0 || !_.keys(memoryObject[key]) || _.keys(memoryObject[key]).length == 0) {
				...
				sizes[key] = {};
				OvermindConsole.recursiveMemoryProfile(memoryObject[key], sizes[key] as RecursiveObject, currentDepth - 1);
			}
		}
	}
