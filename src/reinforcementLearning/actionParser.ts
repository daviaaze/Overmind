...
export type RLAction =
	...
	| ["goTo", RoomPosition]
	| ["attack", Id<Structure | Creep>]
	| ["rangedAttack", Id<Structure | Creep>]
	| ["rangedMassAttack", null]
	| ["heal", Id<Structure | Creep>]
	| ["rangedHeal", Id<Structure | Creep>]
...
export class ActionParser {
	...
	private static parseAction(actor: NeuralZerg, action: RLAction, autoEngage = true): boolean {
		...
		const targ = typeof predicate === "string" ? Game.getObjectById<Id<Structure | Creep>>(predicate) : null;

		switch (command) {
			case "move":
				actor.move(predicate as DirectionConstant);
				break;
			case "goTo":
				if (targ) actor.goTo(targ);
				break;
			case "attack":
				if (targ) actor.attack(targ as Creep);
				break;
			case "rangedAttack":
				if (targ) actor.rangedAttack(targ as Creep);
				break;
			...
			case "heal":
				if (targ) {
					actor.heal(targ as Creep);
				} else if (typeof predicate !== "string") {
					actor.heal(actor);
				}
				break;
			case "rangedHeal":
				if (targ) actor.rangedHeal(targ as Creep);
				break;
			...
			case "avoidAllies":
				...
				break;
			case "maneuver":
				prepareManeuver(predicate, actor);
				break;
			case "noop":
				break;
			default:
	 ...
	 */
	private static parseActions(actors: Record<string, NeuralZerg>, serializedActions: Record<string, RLAction[]>) {
		const receivedOrders: Record<string, boolean> = _.mapValues(actors, () => false);
	 ...
	 */
	private static getAllActors(): Record<string, NeuralZerg> {
	...
	static run() {
		...
		const controllableActors: Record<string, NeuralZerg> = {};
		const uncontrollableActors: Record<string, NeuralZerg> = {};
...
function prepareManeuver(predicate: any, actor: NeuralZerg) {
