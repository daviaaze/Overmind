...
export class ActionParser {
  ...
  private static parseAction (
    ...
    const targ =
			typeof predicate === 'string' ? Game.getObjectById<Id<Structure | Creep>>(predicate) : null
  ...
  private static parseActions (
    actors: Record<string, NeuralZerg>,
    serializedActions: Record<string, RLAction[]>
  ) {
    const receivedOrders: Record<string, boolean> = _.mapValues(
      actors,
      () => false
    )
