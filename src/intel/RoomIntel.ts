...
export class RoomIntel {
  ...
  private static recordPermanentObjects(room: Room): void {
    ...
    if (room.controller?.owner) {
      room.memory[_RM.IMPORTANT_STRUCTURES] = {
        ...
        [_RM_IS.STORAGE]: room.storage
          ? room.storage.pos.coordName
          : undefined,
        [_RM_IS.TERMINAL]: room.terminal
          ? room.terminal.pos.coordName
          : undefined,
  ...
  private static recordControllerInfo(controller: StructureController): void {
    ...
    if (savedController) {
      ...
      savedController[_RM_CTRL.SAFEMODE_COOLDOWN] =
  ...
  static inSafeMode(roomName: string): boolean {
    if (
      ...
      !!Memory.rooms[roomName][_RM.CONTROLLER]
    ) {
      const safemode =
        Memory.rooms[roomName][_RM.CONTROLLER]![_RM_CTRL.SAFEMODE]
  ...
  private static recomputeScoreIfNecessary(room: Room): boolean {
    if (room.memory[_RM.EXPANSION_DATA] == false) {
  ...
  private static updateHarvestData(room: Room): void {
    if (!room.memory[_RM.HARVEST]) {
      room.memory[_RM.HARVEST] = {
        ...
        [_ROLLING_STATS.AVG10K]: _.sumBy(
          room.sources,
          (s) => s.energyCapacity / ENERGY_REGEN_TIME
        ),
        [_ROLLING_STATS.AVG100K]: _.sumBy(
          room.sources,
          (s) => s.energyCapacity / ENERGY_REGEN_TIME
        ),
        [_ROLLING_STATS.AVG1M]: _.sumBy(
          room.sources,
          (s) => s.energyCapacity / ENERGY_REGEN_TIME
        ),
        [_MEM.TICK]: Game.time
    ...
    const harvest = room.memory[_RM.HARVEST]
    for (const source of room.sources) {
      // TODO: this implicitly assumes all energy is harvested by me
      if (source.ticksToRegeneration == 1) {
        ...
        harvest[_ROLLING_STATS.AVG100K] =
          +irregularExponentialMovingAverage(
            dEnergy / dTime,
            harvest[_ROLLING_STATS.AVG10K],
            dTime,
  ...
  private static updateCasualtyData(room: Room): void {
    ...
    const casualtiesCost = room.memory[_RM.CASUALTIES]
      .cost
    for (const tombstone of room.tombstones) {
      if (tombstone.ticksToDecay == 1) {
        ...
        if (
          ...
          casualtiesCost[_ROLLING_STATS.AVG100K] =
            +irregularExponentialMovingAverage(
              dCost / dTime,
              casualtiesCost[_ROLLING_STATS.AVG100K],
              dTime,
          ...
          casualtiesCost[_ROLLING_STATS.AVG1M] =
            +irregularExponentialMovingAverage(
              dCost / dTime,
              casualtiesCost[_ROLLING_STATS.AVG1M],
              dTime,
  ...
  private static recordSafety(room: Room): void {
    ...
    let safety: number
    const safetyData = room.memory[_RM.SAFETY]
  ...
  static roomOwnedBy(roomName: string): string | undefined {
    if (
      ...
      if (Game.time - (Memory.rooms[roomName][_MEM.TICK] || 0) < 25000) {
  ...
  static roomReservedBy(roomName: string): string | undefined {
    if (
      ...
      if (Game.time - (Memory.rooms[roomName][_MEM.TICK] || 0) < 10000) {
        // reservation expires after 10k ticks
        return Memory.rooms[roomName][_RM.CONTROLLER]![
  ...
  static roomReservationRemaining(roomName: string): number {
    if (
      ...
      const timeSinceLastSeen =
        Game.time - (Memory.rooms[roomName][_MEM.TICK] || 0)
    ...
    for (const name in Game.rooms) {
      ...
      if (Game.time >= (room.memory[_MEM.EXPIRATION] || 0)) {
        ...
        const recacheTime = room.owner
          ? OWNED_RECACHE_TIME
          : RECACHE_TIME
        room.memory[_MEM.EXPIRATION] = getCacheExpiration(
          recacheTime,
          250
        )
      }
