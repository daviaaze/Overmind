...
export class RoomIntel {
	...
	private static recordPermanentObjects(room: Room): void {
		...
		room.memory[_RM.PORTALS] = _.map(room.portals, portal => {
			const dest = portal.destination instanceof RoomPosition ? portal.destination.name : portal.destination;
	...
	static safeModeCooldown(roomName: string): number | undefined {
		if (Memory.rooms[roomName]?.[_RM.CONTROLLER]?.[_RM_CTRL.SAFEMODE_COOLDOWN]) {
	...
	private static updateHarvestData(room: Room): void {
		if (!room.memory[_RM.HARVEST]) {
			room.memory[_RM.HARVEST] = {
				...
				[_ROLLING_STATS.AVG10K]: _.sumBy(room.sources, s => s.energyCapacity / ENERGY_REGEN_TIME),
				[_ROLLING_STATS.AVG100K]: _.sumBy(room.sources, s => s.energyCapacity / ENERGY_REGEN_TIME),
				[_ROLLING_STATS.AVG1M]: _.sumBy(room.sources, s => s.energyCapacity / ENERGY_REGEN_TIME),
				[_MEM.TICK]: Game.time
			};
		}
	...
	private static updateCasualtyData(room: Room): void {
		...
		const casualtiesCost = room.memory[_RM.CASUALTIES].cost;
	...
	static getPreviousPos(creep: Creep | Zerg): RoomPosition {
		if (creep.room.memory[_RM.PREV_POSITIONS]?.[creep.id]) {
	...
	static roomOwnedBy(roomName: string): string | undefined {
		if (Memory.rooms[roomName]?.[_RM.CONTROLLER]?.[_RM_CTRL.OWNER]) {
	...
	static roomReservedBy(roomName: string): string | undefined {
		if (Memory.rooms[roomName]?.[_RM.CONTROLLER]?.[_RM_CTRL.RESERVATION]) {
