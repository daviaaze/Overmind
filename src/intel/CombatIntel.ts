...
import { toCreep, type Zerg } from "../zerg/Zerg";
...
export class CombatIntel {
	...
	private findBestExit(
		...
		const allowedExits: Record<string, boolean> = {};
		if (!bestExit) {
			const exitData = Game.map.describeExits(this.room.name);
			for (const direction in exitData) {
				const roomName = exitData[direction as "1" | "3" | "5" | "7"]!;
		...
		if (!bestExit) {
			bestExit = _(exitPositions)
				.sortBy((p: RoomPosition) => -_.sumBy(towers, (t: Structure) => p.getRangeTo(t)))
				.head()!;
		}
	...
	static getFallbackFrom(pos: RoomPosition, fallbackDistance = 2): RoomPosition {
		...
		const [, direction] = _.first(_.sortBy(rangesToExit, pair => pair[0]))!;
		switch (direction) {
			case "left":
	...
	static getMassAttackDamage(attacker: Creep | Zerg, targets = attacker.room.hostiles, checkRampart = true): number {
		...
		return _.sumBy(hostiles, function (hostile) {
	...
	static maxDamageByCreeps(creeps: Creep[]): number {
		return _.sumBy(
			creeps,
			creep =>
				ATTACK_POWER * this.getAttackPotential(creep) + RANGED_ATTACK_POWER * this.getRangedAttackPotential(creep)
		);
	}
	...
	static maxHealingByCreeps(creeps: Creep[]): number {
		return _.sumBy(creeps, creep => this.getHealAmount(creep));
	}
	...
	static getCombatPotentials(creeps: Creep[]): CombatPotentials {
		const attack = _.sumBy(creeps, creep => this.getAttackPotential(creep));
		const rangedAttack = _.sumBy(creeps, creep => this.getRangedAttackPotential(creep));
		const heal = _.sumBy(creeps, creep => this.getHealPotential(creep));
	...
	static maxDamageAtPos(pos: RoomPosition): number {
		...
		const meleeDamage = _.sumBy(hostilesInMeleeRange, hostile => this.getAttackDamage(hostile));
		const hostilesInRange = _.filter(pos.room.dangerousHostiles, creep => pos.getRangeTo(creep) <= 3);
		const rangedDamage = _.sumBy(hostilesInRange, hostile => this.getRangedAttackDamage(hostile));
	...
	static maxHostileHealingTo(creep: Creep): number {
		return this.cache(creep, "maxHostileHealing", () => {
			...
			const rangedHealers = _.filter(
				creep.room.hostiles,
				hostile => hostile.pos.getRangeTo(creep) <= 3 && !neighbors.includes(hostile)
			);
	...
	static getPositionsNearEnemies(hostiles: Creep[], range = 0): RoomPosition[] {
		return _.uniq(_.flatten(_.map(hostiles, hostile => hostile.pos.getPositionsInRange(range, false, true))));
	}
}
