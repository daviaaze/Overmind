...
export class CombatIntel {
  ...
  private findBestExit(
    ...
    if (!bestExit) {
      bestExit = _(exitPositions)
        .sortBy(
          (p: RoomPosition) =>
            -_.sumBy(towers, (t: Structure) => p.getRangeTo(t))
        )
        .head()!
    }
  ...
  static getMassAttackDamageTo(
    ...
    return (
      ...
      this.getRangedAttackPotential(
        isZerg(attacker) ? attacker.creep : attacker
      )
    )
  }
  ...
  static getCombatPotentials(creeps: Creep[]): CombatPotentials {
    ...
    const rangedAttack = _.sumBy(creeps, (creep) =>
  ...
  static maxDamageAtPos(pos: RoomPosition): number {
    ...
    const meleeDamage = _.sumBy(hostilesInMeleeRange, (hostile) =>
    ...
    const rangedDamage = _.sumBy(hostilesInRange, (hostile) =>
  ...
  static maxHostileHealingTo(creep: Creep): number {
    return this.cache(creep, 'maxHostileHealing', () => {
      ...
      const neighborHealing = _.sumBy(neighbors, (neighbor) =>
      ...
      const rangedHealing = _.sumBy(rangedHealers, (healer) =>
  ...
  static avgHostileHealingTo(creeps: Creep[]): number {
    return (
      _.max(
        _.map(creeps, (creep) =>
          CombatIntel.maxHostileHealingTo(creep)
        )
      )! / creeps.length
    )
  }
  ...
  static predictedDamageAmount(
    ...
    if (useHitsPredicted) {
      ...
      const nonToughHits = _.sumBy(target.body, (part) =>
