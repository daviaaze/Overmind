...
import { type CombatMoveOptions, Movement, NO_ACTION, type SwarmMoveOptions } from "../movement/Movement";
...
import { type CombatZerg, DEFAULT_SWARM_TICK_DIFFERENCE } from "./CombatZerg";
...
export class Swarm implements ProtoSwarm {
	private readonly overlord: SwarmOverlord;
	memory: SwarmMemory;
	ref: string;
	...
	formation: Array<Array<CombatZerg | undefined>>; // Relative ordering of the creeps accounting for orientation
	staticFormation: Array<Array<CombatZerg | undefined>>; // Relative ordering of the creeps assuming a TOP orientation
	...
	constructor(overlord: SwarmOverlord, ref: string, creeps: CombatZerg[], width = 2, height = 2) {
		...
		const paddedCreeps: Array<CombatZerg | undefined> = _.clone(creeps);
		...
		this.uniformCreepType = _.uniq(_.filter(_.values(creepScores), score => score != 0)).length <= 1;
		...
		this.roomsByName = _.fromPairs(_.map(this.rooms, room => [room.name, room]));
		this.fatigue = _.max(_.map(this.creeps, creep => creep.fatigue)) ?? 0;
	...
	private getCreepScores(creeps: Array<CombatZerg | undefined>): Record<string, number> {
	...
	private printFormation(formation: Array<Array<CombatZerg | undefined>>): string {
	...
	static cleanMemory(overlord: { swarms: Record<string, Swarm>; memory: SwarmMemory }) {
	...
	private pivot(direction: "clockwise" | "counterclockwise"): number {
		...
		const allMoved = _.every([r1, r2, r3, r4], r => r == OK);
	 ...
	 */
	private getFormationPositionsFromAnchor(anchor: RoomPosition): Record<string, RoomPosition> {
		const formationPositions: Record<string, RoomPosition> = {};
	...
	isInFormation(anchor = this.anchor): boolean {
		...
		return _.every(this.creeps, creep => creep.pos.isEqualTo(formationPositions[creep.name]));
	}
	...
	safelyInRoom(roomName: string): boolean {
		return _.every(this.creeps, creep => creep.safelyInRoom(roomName));
	}
	...
	reorient(includeStructures = true, includeCreeps = false): number {
		...
		const targetRoom = _.find(this.rooms, room => room.owner && !room.my) as Room;
		if (targetRoom) {
	...
	autoCombat(roomName: string, waypoint?: RoomPosition) {
		...
		if (_.some(goals.avoid, goal => this.minRangeTo(goal) <= goal.range)) {
	...
	needsToRecover(recoverThreshold = 0.75, reengageThreshold = 1.0): boolean {
		...
		if (this.memory.recovering) {
			recovering = _.some(this.creeps, creep => creep.hits < creep.hitsMax * reengageThreshold);
		} else {
			recovering = _.some(this.creeps, creep => creep.hits < creep.hitsMax * recoverThreshold);
		}
