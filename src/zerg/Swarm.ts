...
import {
  type CombatMoveOptions,
  Movement,
  NO_ACTION,
  type SwarmMoveOptions
...
export class Swarm implements ProtoSwarm {
  ...
  constructor(
    ...
    this.uniformCreepType =
      _.uniq(_.filter(_.values(creepScores), (score) => score != 0))
        .length <= 1
  ...
  isInFormation(anchor = this.anchor): boolean {
    ...
    return _.every(this.creeps, (creep) =>
  ...
  autoCombat(roomName: string, waypoint?: RoomPosition) {
    ...
    if (
      _.some(goals.avoid, (goal) => this.minRangeTo(goal) <= goal.range)
    ) {
