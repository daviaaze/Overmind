...
export class CombatZerg extends Zerg {
				...
				partner = _(partners)
					.filter(
						partner =>
							...
							Math.abs((this.ticksToLive || CREEP_LIFE_TIME) - (partner.ticksToLive || CREEP_LIFE_TIME)) <=
								tickDifference
					)
					.minBy(partner => Math.abs((this.ticksToLive || CREEP_LIFE_TIME) - (partner.ticksToLive || CREEP_LIFE_TIME)));
			}
			...
			for (const swarmRef in partnersBySwarm) {
				...
				if (
					_.every(
						...
						c => Math.abs((this.ticksToLive || CREEP_LIFE_TIME) - (c.ticksToLive || CREEP_LIFE_TIME)) <= tickDifference
					)
				) {
	...
	autoMelee(possibleTargets = this.room.hostiles) {
		const target =
			CombatTargeting.findBestCreepTargetInRange(this, 1, possibleTargets) ||
	...
	autoRanged(possibleTargets = this.room.hostiles, allowMassAttack = true) {
		const target =
			CombatTargeting.findBestCreepTargetInRange(this, 3, possibleTargets) ||
	...
	autoBunkerCombat(roomName: string) {
		...
		const siegingCreeps = this.room.hostiles.filter(creep =>
			_.some(creep.pos.neighbors, pos => this.colony && insideBunkerBounds(pos, this.colony))
		);
	...
	needsToRecover(
		recoverThreshold = CombatIntel.minimumDamageTakenMultiplier(this.creep) < 1 ? 0.85 : 0.75,
