...
export class Zerg {
  ...
  get ticksUntilSpawned (): number | undefined {
    if (this.spawning) {
      const spawner = this.pos.lookFor('structure').find(w => w.structureType ===
        STRUCTURE_SPAWN
      ) as StructureSpawn
      if (spawner && spawner.spawning) {
        ...
        console.log(
					`Error determining ticks to spawn for ${this.name} @ ${this.pos.x}, ${this.pos.y}!`
        )
      }
    }
  }
    ...
    for (const resourceType in this.carry) {
      if (
        resourceType != RESOURCE_ENERGY &&
				(this.carry[(resourceType as ResourceConstant)] || 0) > 0
      ) {
        return true
      }
    }
  ...
  /**
	 ...
	 */
  kite (
    avoidGoals: Array<RoomPosition | HasPos> = this.room.hostiles,
    options: MoveOptions = {}
  ): number | undefined {
    _.defaults(options, {
      fleeRange: 5
