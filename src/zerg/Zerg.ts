...
import { Movement, type MoveOptions } from "../movement/Movement";
...
export class Zerg {
	...
	lifetime: number;
	actionLog: Record<string, boolean>; // Tracks the actions that a creep has completed this tick
	blockMovement: boolean; // Whether the zerg is allowed to move or not
	private _task: Task | null | undefined; // Cached Task object that is instantiated once per tick and on change
	...
	get ticksUntilSpawned(): number {
		if (this.spawning) {
			const spawner = this.pos.lookFor("structure").find(w => w.structureType === STRUCTURE_SPAWN) as StructureSpawn;
			if (spawner && spawner.spawning) {
				return spawner.spawning.remainingTime;
			} else {
				// Shouldn't ever get here
				console.log(`Error determining ticks to spawn for ${this.name} @ ${this.pos.x}, ${this.pos.y}!`);
			}
		}
		return Infinity;
	}
	...
	get hasMineralsInCarry(): boolean {
		for (const resourceType in this.carry) {
			if (resourceType != RESOURCE_ENERGY && (this.carry[resourceType as ResourceConstant] || 0) > 0) {
				return true;
			}
		}
		return false;
	...
	get boostCounts(): Record<string, number> {
	 ...
	 */
	kite(avoidGoals: Array<RoomPosition | HasPos> = this.room.hostiles, options: MoveOptions = {}): number | undefined {
		_.defaults(options, {
			fleeRange: 5
		});
	...
	private defaultFleeGoals() {
		let fleeGoals: Array<RoomPosition | HasPos> = [];
		fleeGoals = fleeGoals
	 ...
	 */
	flee(
		avoidGoals: Array<RoomPosition | HasPos> = this.room.fleeDefaults,
