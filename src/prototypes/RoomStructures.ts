...
const roomStructureIDs: Record<string, Record<string, string[]>> = {};
// const roomStructuresExpiration: Record<string, number> = {}
...
// //       ),
// //       (structures: Structure[]) => structures.map((s) => s.id)
// //     )
// //   }
// // }
		...
		if (!this._walkableRamparts) {
			...
			if (roomStructureIDs[this.name].walkableRamparts) {
				return (this._walkableRamparts = _.compact(
					_.map(roomStructureIDs[this.name].walkableRamparts, Game.getObjectById)
				));
			} else {
				const walkableRamparts = _.filter(this.ramparts, (r: StructureRampart) => r.pos.isWalkable(true));
...
Object.defineProperty(Room.prototype, "rechargeables", {
	get() {
		if (!this._rechargeables) {
			this._rechargeables = [...this.storageUnits, ...this.droppedEnergy, ...this.tombstones];
		}
