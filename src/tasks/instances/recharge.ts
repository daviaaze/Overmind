...
export class TaskRecharge extends Task {
	get target(): rechargeTargetType {
		return super.target as rechargeTargetType;
	}
	...
	private rechargeRateForCreep(creep: Zerg, obj: rechargeObjectType): number | false {
		if (creep.colony?.hatchery?.battery && obj.id == creep.colony.hatchery.battery.id && creep.roleName != "queen") {
			return false; // only queens can use the hatchery battery
		}
		let amount = isResource(obj) ? obj.amount : obj.store.energy;
		...
		const otherTargeters = _.filter(
			...
			zerg =>
				!!zerg &&
				zerg.memory._task &&
				(zerg.memory._task.name == withdrawTaskName || zerg.memory._task.name == pickupTaskName)
		);
