...
interface targetType { ref: string, pos: ProtoPos } // overwrite this variable to specify more precise typing
...
export abstract class Task {
  ...
  get targetManifest (): Array<RoomObject | null> {
    ...
    return targetRefs.map((ref) => deref(ref))
  }
  ...
  moveToTarget (range = this.settings.targetRange): number {
    return this.creep.goTo(this.targetPos, { range })
  }
  ...
  finish (): void {
    ...
    if (this.creep) {
      ...
      log.debug(
				`No creep executing ${this.name}! Proto: ${JSON.stringify(this.proto)}`
      )
    }
  }
}
