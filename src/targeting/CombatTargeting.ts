...
export class CombatTargeting {
	...
	static findBestCreepTargetInRange(zerg: Zerg, range: number, targets = zerg.room.hostiles): Creep | undefined {
		const nearbyHostiles = targets.filter(c => zerg.pos.inRangeToXY(c.pos.x, c.pos.y, range));
	...
	static findBestStructureTargetInRange(zerg: Zerg, range: number, allowUnowned = true): Structure | undefined {
		let nearbyStructures = zerg.room.hostileStructures.filter(s => zerg.pos.inRangeToXY(s.pos.x, s.pos.y, range));
	...
	static findClosestReachable(pos: RoomPosition, targets: Array<Creep | Structure>): Creep | Structure | undefined {
			...
			friend => {
				...
				for (const hostile of friend.pos.findInRange(friend.room.hostiles, 3)) {
					if (hostile.pos.isNearTo(friend)) {
						...
						friend.hitsPredicted -=
							attackProbability * (CombatIntel.getAttackDamage(hostile) + CombatIntel.getRangedAttackDamage(hostile));
					}
				}
