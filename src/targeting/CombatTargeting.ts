...
export class CombatTargeting {
  ...
  static findBestCreepTargetInRange (
    ...
    const nearbyHostiles = targets.filter((c) =>
  ...
  static findBestStructureTargetInRange (
    ...
    let nearbyStructures = zerg.room.hostileStructures.filter((s) =>
  ...
  static findBestSwarmStructureTarget (
    ...
    const ret = PathFinder.search(swarm.anchor, approach, {
      ...
      roomCallback: (rn) => {
        ...
        const matrix = Pathing.getSwarmTerrainMatrix(
          roomName,
          swarm.width,
          swarm.height
        ).clone()
        for (const barrier of room.barriers) {
          ...
          const cost =
						...
						randomFactor
          const setPositions = Pathing.getPosWindow(
            barrier.pos,
            -swarm.width,
            -swarm.height
          )
          for (const pos of setPositions) {
            matrix.set(
              pos.x,
              pos.y,
              Math.max(cost, matrix.get(pos.x, pos.y))
            )
          }
        }
