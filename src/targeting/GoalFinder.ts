...
export class GoalFinder {
	...
	static skirmishGoals(creep: CombatZerg): {
		...
		const analysis = {} as Record<string, SkirmishAnalysis | undefined>;
		...
		const nearbyRating = _.sumBy(creep.pos.findInRange(room.creeps, 6), c => CombatIntel.rating(c));
		const braveMode = creep.hits * (nearbyRating / myRating) * 0.5 > creep.hitsMax;
		...
		for (const target of approachTargets) {
			...
			if (data && (data.advantage || braveMode)) {
				...
				if (!preferCloseCombat && (data.attack > 0 || data.rangedAttack > myRangedAttack)) {
					...
					avoid.push({ pos: target.pos, range });
				}
				approach.push({ pos: target.pos, range });
			}
		}
	...
	static swarmCombatGoals(
		...
		const myAttack = _.sumBy(swarm.creeps, creep => CombatIntel.getAttackDamage(creep));
		const myRangedAttack = _.sumBy(swarm.creeps, creep => CombatIntel.getRangedAttackDamage(creep));
		const myHealing = _.sumBy(swarm.creeps, creep => CombatIntel.getHealAmount(creep));
		...
		// const myRating = _.sumBy(swarm.creeps, (creep) =>
		...
		for (const i in hostileSwarms) {
			...
			const attack = _.sumBy(hostiles, creep => CombatIntel.getAttackDamage(creep));
			const rangedAttack = _.sumBy(hostiles, creep => CombatIntel.getRangedAttackDamage(creep));
			const healing = _.sumBy(hostiles, creep => CombatIntel.getHealAmount(creep));
			const damageMultiplier = CombatIntel.minimumDamageMultiplierForGroup(hostiles);

			const canPopShield =
				(attack + rangedAttack + CombatIntel.towerDamageAtPos(swarm.anchor)) * myDamageMultiplier >
				_.min(_.map(swarm.creeps, creep => 100 * creep.getActiveBodyparts(TOUGH)))!;

			const isRetreating =
				_.sumBy(hostiles, creep => +CombatIntel.isRetreating(creep, swarm.anchor)) / hostiles.length >= 0.5;

			const isApproaching =
				_.sumBy(hostiles, creep => +CombatIntel.isApproaching(creep, swarm.anchor)) / hostiles.length >= 0.5;

			const advantage =
				healing == 0 ||
				attack + rangedAttack == 0 ||
				myAttack + myRangedAttack + myHealing / myDamageMultiplier > attack + rangedAttack + healing / damageMultiplier;
			...
			for (const hostile of hostiles) {
				if (canPopShield && hostile.pos.lookForStructure(STRUCTURE_RAMPART)) {
					...
					avoid.push({ pos: hostile.pos, range });
				} else {
					if (advantage) {
						...
						approach.push({ pos: hostile.pos, range });
					} else {
	...
	// static structureGoals (creep: CombatZerg): {
	static structureGoals(): {
		approach: PathFinderGoal[];
		avoid: PathFinderGoal[];
	} {
		const approach: PathFinderGoal[] = [];
