...
export class GoalFinder {
  ...
  static skirmishGoals (creep: CombatZerg): {
    ...
    const nearbyRating = _.sumBy(creep.pos.findInRange(room.creeps, 6), (c) =>
      CombatIntel.rating(c)
    )
    const braveMode =
			creep.hits * (nearbyRating / myRating) * 0.5 > creep.hitsMax
    ...
    for (const hostile of room.hostiles) {
      ...
      analysis[hostile.id] = {
        ...
        isRetreating: CombatIntel.isRetreating(
          hostile,
          RoomIntel.getPreviousPos(creep)
        ),
    ...
    for (const target of approachTargets) {
      ...
      if (data && (data.advantage || braveMode)) {
        ...
        if (
          ...
          range =
						creep.pos.getRangeTo(target) == 3 && data.isRetreating
              ? 2
              : 3
  ...
  static swarmCombatGoals (
    ...
    const myAttack = _.sumBy(swarm.creeps, (creep) =>
      CombatIntel.getAttackDamage(creep)
    )
    const myRangedAttack = _.sumBy(swarm.creeps, (creep) =>
      CombatIntel.getRangedAttackDamage(creep)
    )
    const myHealing = _.sumBy(swarm.creeps, (creep) =>
    ...
    for (const i in hostileSwarms) {
      ...
      const attack = _.sumBy(hostiles, (creep) =>
        CombatIntel.getAttackDamage(creep)
      )
      const rangedAttack = _.sumBy(hostiles, (creep) =>
        CombatIntel.getRangedAttackDamage(creep)
      )
      const healing = _.sumBy(hostiles, (creep) =>
        CombatIntel.getHealAmount(creep)
      )
