...
import { type StructureLayout, type StructureMap } from "../roomPlanner/RoomPlanner";
...
export class Visualizer {
	...
	static drawStructureMap(structureMap: StructureMap): void {
		...
		const vis: Record<string, RoomVisual> = {};
	...
	static drawLayout(layout: StructureLayout, anchor: RoomPosition, opts: { opacity: number } = { opacity: 0.5 }): void {
	...
	static infoBox(
		...
		const height = CHAR_HEIGHT * (content.length > 0 ? content.length : 1);
	...
	static barGraph(
		...
		if (mode == "percent") {
			...
			vis.text(`${num}/${den}`, pos.x + width / 2, pos.y - 0.1 * CHAR_HEIGHT, this.textStyle(1, { align: "center" }));
		}
	}
	...
	static summary(): void {
		this.text(
			`Colonies: ${_.keys(Overmind.colonies).length} | Creeps: ${_.keys(Game.creeps).length}`,
			{
				x: 1,
				y: 10
			},
