...
const _logoComponents: Record<string, { style: PolyStyle; points: Array<[number, number]> }> = {
	black: {
...
const logoScale = 0.6;
export const logoComponents = _.mapValues(_logoComponents, c => ({
	style: c.style,
	points: _.map(c.points, xy => [logoX + logoScale * xy[0], logoY + logoScale * xy[1]])
})) as Record<string, { style: PolyStyle; points: Array<[number, number]> }>;
...
export const logoText = _.mapValues(_logoText, function (c) {
	const ret = {
		style: c.style,
		points: _.map(c.points, xy => [textX + textScale * (offset + xy[0]), textY + textScale * xy[1]])
	};
